...
public class CashReceiptService implements ICashReceiptService {
		...
		try {
			switch (criteria.getReferenceType()) {
				...
				case LIFE_PROPOSAL: {
					...
					// WorkflowCriteria(WorkflowReferenceType.LIFE_PROPOSAL,
	...
	public List<LifeCashReceiptListReportDTO> confirmLifeProposalsForCashReceipt(List<CashReceiptDTO> cashReceiptDTOList, WorkflowReferenceType type, PaymentDTO payment,
		...
		WorkFlowDTO workFlowDTO = new WorkFlowDTO(null, remark, WorkflowTask.PAYMENT, WorkflowReferenceType.LIFE_PROPOSAL, currentUser, responsiblePerson);
		for (CashReceiptDTO cashReceiptDTO : cashReceiptDTOList) {
			...
			List<Payment> paymentList = lifeProposalService.confirmLifeProposal(proposal, workFlowDTO, payment, userProcessService.getLoginUser().getBranch(), "", false);
