...
public class AGENT_Filter extends BasicDAO implements IAGENT_Filter {
	...
	public List<AGNT001> find(CRIA001 item, String value) {
		...
		StringBuffer bufferer = new StringBuffer("SELECT c.id, c.codeNo, c.liscenseNo, ");
		...
		bufferer.append("FROM Agent c LEFT OUTER JOIN c.residentAddress.township t ");
		switch (item) {
			case FIRSTNAME: {
		...
		for (Object[] obj : objectList) {
			...
			branch = (String) obj[6];
			result.add(new AGNT001(id, agentCode, licenseNo, name, idNo, address, branch));
		}
	...
	public List<AGNT001> find(int maxResult) {
		...
		bufferer.append("CONCAT(TRIM(c.residentAddress.residentAddress), ', ', TRIM(t.name), ', ', TRIM(t.province.name), ', ', TRIM(t.province.country.name)), ");
		bufferer.append("c.branch.name ");
		bufferer.append("FROM Agent c LEFT OUTER JOIN c.residentAddress.township t  WHERE c.status = true ORDER BY c.id DESC");
		Query query = em.createQuery(bufferer.toString());
