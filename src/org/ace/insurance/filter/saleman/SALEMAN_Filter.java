...
public class SALEMAN_Filter extends BasicDAO implements ISALEMAN_Filter {
		...
		switch (item) {
			...
			default: {
				bufferer.append("WHERE  c.status = '1' AND CONCAT(c.name.firstName, ' ', c.name.middleName, ' ', c.name.lastName) like '%" + value + "%' "
						+ "OR c.status = '1' AND CONCAT(c.name.firstName, ' ', c.name.middleName, c.name.lastName) like '%" + value + "%'");
			}
		}
	...
	public List<SAMN001> find(int maxResult) {
		...
		StringBuffer bufferer = new StringBuffer(
				"SELECT c.id, c.codeNo, c.licenseNo, c.name, c.idNo, c.address, c.branch.name FROM SaleMan c WHERE c.status=true ORDER BY c.id DESC");
		Query query = em.createQuery(bufferer.toString());
