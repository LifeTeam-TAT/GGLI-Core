...
public class LifeClaimService implements ILifeClaimService {
	...
	private void setIDPrefixForInsert(LifeClaim lifeClaim) {
		String lifeClaimIdPrefix = customIDGenerator.getPrefix(LifeClaim.class, true);
		String lifeClaimBeneficiaryPrefix = customIDGenerator.getPrefix(LifeClaimBeneficiary.class, false);
		String lifeClaimInsuredPersonPrefix = customIDGenerator.getPrefix(LifeClaimInsuredPerson.class, false);

		lifeClaim.setPrefix(lifeClaimIdPrefix);
		lifeClaim.getClaimInsuredPerson().setPrefix(lifeClaimInsuredPersonPrefix);
		if (Utils.isNotNull(lifeClaim.getSuccessor())) {
			lifeClaim.getSuccessor().setPrefix(lifeClaimBeneficiaryPrefix);
		}
		for (LifeClaimInsuredPersonBeneficiary lcipb : lifeClaim.getClaimInsuredPersonBeneficiaryList()) {
			lcipb.setPrefix(lifeClaimBeneficiaryPrefix);
		}
	}
	...
	public void addNewLifeClaim(LifeClaim lifeClaim, WorkFlowDTO workflowDTO) {
		try {
			Product product = lifeClaim.getLifePolicy().getPolicyInsuredPersonList().get(0).getProduct();
			String claimNo = null;
			if (ProductIDConfig.isPublicLife(product)) {
				claimNo = SystemConstants.ENDOWMENT_LIFE_CLAIM_NO;
			} else if (ProductIDConfig.isGroupLife(product)) {
				claimNo = SystemConstants.GROUP_LIFE_CLAIM_NO;
			} else if (ProductIDConfig.isShortEndowmentLife(product)) {
				claimNo = SystemConstants.SHORT_TERM_ENDOWMENT_LIFE_CLAIM_NO;
			}

			String claimRequestId = customIDGenerator.getNextId(claimNo, null, true);
			...
			setIDPrefixForInsert(lifeClaim);
	...
	public List<Payment> confirmLifeClaim(LifeClaim lifeClaim, WorkFlowDTO workflowDTO, PaymentDTO paymentDTO) {
		...
		try {
			...
			if (lifeClaim.isDeathClaim()) {
				...
				for (LifeClaimBeneficiary beneficiary : lifeClaim.getClaimInsuredPersonBeneficiaryList()) {
					...
					if (insuredPersonBeneficiary.getClaimSuccessor() == null) {
						refundNo = customIDGenerator.getNextId(SystemConstants.LIFE_CLAIM_REFUND_NO, null, false);
			...
			paymentService.prePaymentAndTlfLifeClaim(paymentList, lifeClaimBeneficiary, lifeClaim, premium);
		...
		try {
			...
			if (claimBeneficiary.isClaimInsuredPersonBeneficiary()) {
				...
				lifePolicyDAO.updateBeneficiaryClaimStatusById(claimBeneficiary.castClaimInsuredPersonBeneficiary().getPolicyInsuredPersonBeneficiaries().getId(),
						ClaimStatus.PAID);
				branch = claimBeneficiary.castClaimInsuredPersonBeneficiary().getLifeClaim().getBranch();
				premium = claimBeneficiary.castClaimInsuredPersonBeneficiary().getDeficitPremium();
				accountName = claimBeneficiary.castClaimInsuredPersonBeneficiary().getLifeClaim().getLifePolicy().getProductGroup().getAccountCode();
			...
			if (claimBeneficiary.isSuccessor()) {
				...
				branch = claimBeneficiary.castLifeClaimSuccessor().getLifeClaim().getBranch();
				premium = claimBeneficiary.castLifeClaimSuccessor().getDeficitPremium();
				accountName = claimBeneficiary.castLifeClaimSuccessor().getLifeClaim().getLifePolicy().getProductGroup().getAccountCode();
				policyReferenceType = PolicyReferenceType.LIFE_CLAIM;
			}
			...
			if (claimBeneficiary.isDisabilityPerson()) {
				...
				branch = claimBeneficiary.castLifeDisabilityPerson().getClaimInsuredPersonLinkClaim().getBranch();
			...
			paymentService.activatePaymentAndTLF(paymentList, null, null, currencyCode, null);
