...
import org.ace.insurance.life.policyEditHistory.PolicyInsuredPersonAddonEditHistory;
import org.ace.insurance.life.policyEditHistory.PolicyInsuredPersonAttachmentEditHistory;
...
import org.ace.insurance.life.policyEditHistory.PolicyInsuredPersonKeyFactorValueEditHistory;
...
public class LifePolicyEditHistoryService extends BaseService implements ILifePolicyEditHistoryService {
	...
	public void addNewLifePolicy(LifePolicy lifePolicy, PolicyStatus status, PolicyHistoryEntryType entryType) {
		try {
			...
			lifePolicyHistory.setSaleMan(lifePolicy.getSaleMan());
	...
	private void setPrefix(LifePolicyEditHistory lifePolicy) {
		String lPolicyPrefix = customIDGenerator.getPrefix(LifePolicyEditHistory.class, true);
		lifePolicy.setPrefix(lPolicyPrefix);
		List<LifePolicyAttachmentEditHistory> lpAttachmentList = lifePolicy.getAttachmentList();
		if (lpAttachmentList != null && !lpAttachmentList.isEmpty()) {
			for (LifePolicyAttachmentEditHistory lpAtt : lpAttachmentList) {
				String attPrefix = customIDGenerator.getPrefix(LifePolicyAttachmentEditHistory.class, false);
				lpAtt.setPrefix(attPrefix);
			}
		}
		for (PolicyInsuredPersonEditHistory pi : lifePolicy.getPolicyInsuredPersonList()) {
			String insPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonEditHistory.class, false);
			pi.setPrefix(insPrefix);
			List<PolicyInsuredPersonAttachmentEditHistory> piAttachmentList = pi.getAttachmentList();
			if (piAttachmentList != null && !piAttachmentList.isEmpty()) {
				for (PolicyInsuredPersonAttachmentEditHistory att : piAttachmentList) {
					String attPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonAttachmentEditHistory.class, false);
					att.setPrefix(attPrefix);
				}
			}
			List<PolicyInsuredPersonBeneficiariesEditHistory> insBeneList = pi.getPolicyInsuredPersonBeneficiariesList();
			if (insBeneList != null && !insBeneList.isEmpty()) {
				for (PolicyInsuredPersonBeneficiariesEditHistory bHistory : insBeneList) {
					String insBenePrefix = customIDGenerator.getPrefix(PolicyInsuredPersonBeneficiariesEditHistory.class, false);
					bHistory.setPrefix(insBenePrefix);
				}
			}
			List<PolicyInsuredPersonAddonEditHistory> insAddOnList = pi.getPolicyInsuredPersonAddOnList();
			if (insAddOnList != null && !insAddOnList.isEmpty()) {
				for (PolicyInsuredPersonAddonEditHistory vehAddOn : insAddOnList) {
					String insAddOnPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonAddonEditHistory.class, false);
					vehAddOn.setPrefix(insAddOnPrefix);
				}
			}
			List<PolicyInsuredPersonKeyFactorValueEditHistory> policyKeyFactorValueList = pi.getPolicyInsuredPersonkeyFactorValueList();
			if (policyKeyFactorValueList != null && !policyKeyFactorValueList.isEmpty()) {
				for (PolicyInsuredPersonKeyFactorValueEditHistory kf : policyKeyFactorValueList) {
					String kfPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonKeyFactorValueEditHistory.class, false);
		...
		try {
			...
			for (LifePolicyEditHistory lp : policyList) {
				...
				if (ProductIDConfig.isPublicLife(product)) {
					policyNo = customIDGenerator.getNextId(SystemConstants.ENDOWMENT_LIFE_POLICY_NO, productCode, true);
