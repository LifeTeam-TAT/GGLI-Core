...
public class LifePolicyHistoryService extends BaseService implements ILifePolicyHistoryService {
	...
	private void setPrefix(LifePolicyHistory lifePolicy) {
		String lPolicyPrefix = customIDGenerator.getPrefix(LifePolicyHistory.class, true);
		lifePolicy.setPrefix(lPolicyPrefix);
		List<LifePolicyAttachmentHistory> lpAttachmentList = lifePolicy.getAttachmentList();
		if (lpAttachmentList != null && !lpAttachmentList.isEmpty()) {
			for (LifePolicyAttachmentHistory lpAtt : lpAttachmentList) {
				String attPrefix = customIDGenerator.getPrefix(LifePolicyAttachmentHistory.class, false);
				lpAtt.setPrefix(attPrefix);
			}
		}
		for (PolicyInsuredPersonHistory pi : lifePolicy.getPolicyInsuredPersonList()) {
			String insPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonHistory.class, false);
			pi.setPrefix(insPrefix);
			List<PolicyInsuredPersonAttachmentHistory> piAttachmentList = pi.getAttachmentList();
			if (piAttachmentList != null && !piAttachmentList.isEmpty()) {
				for (PolicyInsuredPersonAttachmentHistory att : piAttachmentList) {
					String attPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonAttachmentHistory.class, false);
					att.setPrefix(attPrefix);
				}
			}
			List<PolicyInsuredPersonBeneficiariesHistory> insBeneList = pi.getPolicyInsuredPersonBeneficiariesList();
			if (insBeneList != null && !insBeneList.isEmpty()) {
				for (PolicyInsuredPersonBeneficiariesHistory bHistory : insBeneList) {
					String insBenePrefix = customIDGenerator.getPrefix(PolicyInsuredPersonBeneficiariesHistory.class, false);
					bHistory.setPrefix(insBenePrefix);
				}
			}
			List<PolicyInsuredPersonAddonHistory> insAddOnList = pi.getPolicyInsuredPersonAddOnList();
			if (insAddOnList != null && !insAddOnList.isEmpty()) {
				for (PolicyInsuredPersonAddonHistory vehAddOn : insAddOnList) {
					String insAddOnPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonAddonHistory.class, false);
					vehAddOn.setPrefix(insAddOnPrefix);
				}
			}
			List<PolicyInsuredPersonKeyFactorValueHistory> policyKeyFactorValueList = pi.getPolicyInsuredPersonkeyFactorValueList();
			if (policyKeyFactorValueList != null && !policyKeyFactorValueList.isEmpty()) {
				for (PolicyInsuredPersonKeyFactorValueHistory kf : policyKeyFactorValueList) {
					String kfPrefix = customIDGenerator.getPrefix(PolicyInsuredPersonKeyFactorValueHistory.class, false);
