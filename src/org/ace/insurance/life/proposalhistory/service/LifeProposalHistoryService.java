...
import org.ace.insurance.life.proposalhistory.LifeProposalInsuredPersonAddonHistory;
...
import org.ace.insurance.life.proposalhistory.LifeProposalInsuredPersonKeyFactorValueHistory;
...
public class LifeProposalHistoryService extends BaseService implements ILifeProposalHistoryService {
	...
	public void addNewLifeProposalHistory(LifeProposal lifeProposal) {
		try {                    
			...
			setIDPrefixForUpdate(mpHistory);
	...
	private void setIDPrefixForUpdate(LifeProposalHistory lifeProposal) {
		String mProposalPrefix = customIDGenerator.getPrefix(LifeProposalHistory.class, true);
		String insuredPersonPrefix = customIDGenerator.getPrefix(LifeProposalInsuredPersonHistory.class, false);
		String insuredPersonBeneficiaryPrefix = customIDGenerator.getPrefix(LifeProposalInsuredPersonBeneficiariesHistory.class, false);
		String isPesAddOnPrefix = customIDGenerator.getPrefix(LifeProposalInsuredPersonKeyFactorValueHistory.class, false);
		String insuredPersonAddOnPrefix = customIDGenerator.getPrefix(LifeProposalInsuredPersonAddonHistory.class, false);

		lifeProposal.setPrefix(mProposalPrefix);
		for (LifeProposalInsuredPersonHistory pv : lifeProposal.getLifeProposalInsuredPersonHistoryList()) {
			pv.setPrefix(insuredPersonPrefix);
			for (LifeProposalInsuredPersonKeyFactorValueHistory vhKf : pv.getLifeProposalInsuredPersonKeyFactorValueHistoryList()) {
				vhKf.setPrefix(isPesAddOnPrefix);
			}
			List<LifeProposalInsuredPersonAddonHistory> insuredPersonAddOnList = pv.getLifeProposalInsuredPersonAddonHistoryList();
			if (insuredPersonAddOnList != null && !insuredPersonAddOnList.isEmpty()) {
				for (LifeProposalInsuredPersonAddonHistory insuredPersonAddOn : insuredPersonAddOnList) {
					insuredPersonAddOn.setPrefix(insuredPersonAddOnPrefix);
				}
			}
			List<LifeProposalInsuredPersonBeneficiariesHistory> insuredPersonBeneficiaryList = pv.getLifeProposalInsuredPersonBeneficiariesHistoryList();
			if (insuredPersonBeneficiaryList != null && !insuredPersonBeneficiaryList.isEmpty()) {
				for (LifeProposalInsuredPersonBeneficiariesHistory insuredpersonbeneficiary : insuredPersonBeneficiaryList) {
