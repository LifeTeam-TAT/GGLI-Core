...
import org.ace.insurance.web.manage.medical.claim.factory.DeathClaimFactory;
import org.ace.insurance.web.manage.medical.claim.factory.HospitalizedClaimFactory;
import org.ace.insurance.web.manage.medical.claim.factory.MedicalClaimProposalFactory;
import org.ace.insurance.web.manage.medical.claim.factory.MedicationClaimFactory;
import org.ace.insurance.web.manage.medical.claim.factory.OperationClaimFactory;
...
public class MedicalClaimProposalDAO extends BasicDAO implements IMedicalClaimProposalDAO {
	...
	public List<MC001> findByEnquiryCriteria(ClaimEnquiryCriteria criteria) throws DAOException {
		...
		MedicalClaimProposalDTO claimProposalDTO = new MedicalClaimProposalDTO();
		String policyNo;
		String saleMan;
		String agent;
		String branch;
		...
		try {
			...
			for (Object[] b : objectList) {
				...
				if (claimProposal.getId() != null) {
					claimProposalDTO = MedicalClaimProposalFactory.createMedicalClaimProposalDTO(claimProposal);
					...
					for (MedicalClaim mc : claimProposal.getMedicalClaimList()) {
						...
						if (mc.getClaimRole().equals(MedicalClaimRole.HOSPITALIZED_CLAIM) && mc.isApproved()) {
							...
							totalClaimAmount += claimProposalDTO.getHospitalizedClaimDTO().getHospitalizedAmount();
						}
						if (mc.getClaimRole().equals(MedicalClaimRole.OPERATION_CLAIM) && mc.isApproved()) {
							...
							totalClaimAmount += claimProposalDTO.getOperationClaimDTO().getOperationFee();
						}
						if (mc.getClaimRole().equals(MedicalClaimRole.MEDICATION_CLAIM) && mc.isApproved()) {
							...
							totalClaimAmount += claimProposalDTO.getMedicationClaimDTO().getMedicationFee();
						}
						if (mc.getClaimRole().equals(MedicalClaimRole.DEATH_CLAIM) && mc.isApproved()) {
							...
							totalClaimAmount += claimProposalDTO.getDeathClaimDTO().getDeathClaimAmount();
						}
					}

				}
				if (claimProposal.getSaleman() == null) {
					saleMan = "";
				} else {
					SaleMan s = (SaleMan) claimProposal.getSaleman();
				...
				resultMap.put(id, new MC001(id, policyNo, saleMan, agent, branch, unit, totalClaimAmount, submittedDate, claimRequestNo, receivedNo));
			}
			em.flush();
			System.gc();
		} catch (PersistenceException pe) {
		...
		try {
			...
			for (Object[] b : objectList) {
				...
				if (claimProposal.getId() != null) {
					...
					for (MedicalClaim mc : claimProposal.getMedicalClaimList()) {
						if (mc.getClaimRole().equals(MedicalClaimRole.HOSPITALIZED_CLAIM) && mc.isApproved()) {
							hospitalizedClaimDTO = HospitalizedClaimFactory.createHospitalizedClaimDTO(findHospitalizedClaimById(mc.getId()));
							totalClaimAmount += hospitalizedClaimDTO.getHospitalizedAmount();
						}
						if (mc.getClaimRole().equals(MedicalClaimRole.DEATH_CLAIM) && mc.isApproved()) {
							deathClaimDTO = DeathClaimFactory.createDeathClaimDTO(findDeathClaimById(mc.getId()));
							totalClaimAmount += deathClaimDTO.getDeathClaimAmount();
						}
					}
				...
				resultMap.put(claimProposalId, new MC001(claimProposalId, policyNo, saleMan, agent, branch, unit, totalClaimAmount, submittedDate, claimRequestNo, receivedNo));
			}
			em.flush();
			System.gc();
		} catch (PersistenceException pe) {
	...
	public List<MedicalClaimMonthlyReport> findMedicalClaimMonthlyReport(MonthlyReportCriteria criteria) throws DAOException {
		...
		try {
			...
			StringBuffer queryString = new StringBuffer();
			queryString
					.append("SELECT mc.submittedDate ,mc.claimRequestId , CONCAT(mc.medicalPolicy.policyInsuredPerson.name.firstName , ' ' ,mc.medicalPolicy.policyInsuredPerson.name.firstName, ' ' ,mc.medicalPolicy.policyInsuredPerson.name.firstName), "
