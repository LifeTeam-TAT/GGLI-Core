...
public class MedicalHospitalizedClaimDAO extends BasicDAO implements IMedicalHospitalizedClaimDAO {
	 ...
	 * query.executeUpdate(); updateProcessLog(TableName.HOSPITALIZED_PERSON,
	 * hospitalizationPerson.getId());
	 * updateProcessLog(TableName.MEDICALCLAIM_INSUREDPERSON,
	 * hospitalizationPerson.getId()); } catch (PersistenceException pe) { throw
	 ...
	 * em.flush();System.gc(); insertProcessLog(TableName.MEDICALCLAIM_ATTACHMENT,
	 * att.getId()); } HospitalizationPerson pin =
	 ...
	 * ().getClaimInsuredPersonAttachmentList()) { em.flush();System.gc(); em.persist(att);
	 * insertProcessLog(TableName.MEDICALCLAIM_INSUREDPERSON_ATTACHMENT,
	 * att.getId()); } updateProcessLog(TableName.HOSPITALIZEDCLAIM,
	 * hospitalizedClaim.getId()); em.flush();System.gc(); } catch (PersistenceException pe)
	 ...
	 * em.flush();System.gc(); insertProcessLog(TableName.HOSPITALIZEDCLAIMSURVEY,
	 * hospitalizedClaimSurvey.getId()); } catch (PersistenceException pe) {
	 * throw translate("Failed to insert hospitalized claim survey", pe); } }
	 * 
	 ...
	 * result = em.find(SuccessorClaimBeneficiary.class, id); em.flush();System.gc(); }
	 ...
	 * { result = em.find(HospitalizedClaimBeneficiary.class, id); em.flush();System.gc(); }
	 * catch (PersistenceException pe) { throw
	 * translate("Failed to find MedicalProposal", pe); } return result; }
	 */
