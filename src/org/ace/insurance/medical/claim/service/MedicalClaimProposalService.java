...
public class MedicalClaimProposalService implements IMedicalClaimProposalService {
	...
	public HospitalizedClaimDTO findHospitalizedClaimById(String id) {
		...
		try {
			HospitalizedClaim hospitalizedClaim = claimDAO.findHospitalizedClaimById(id);
	...
	public MedicationClaimDTO findMedicationClaimById(String id) {
		...
		try {
			...
			throw new SystemException(e.getErrorCode(), "Faield to find a MedicationClaim(ID : " + id + ")", e);
		}
		return result;
	}
	...
	public OperationClaimDTO findOperationClaimById(String id) {
		...
		try {
			...
			throw new SystemException(e.getErrorCode(), "Faield to find a OperationClaim(ID : " + id + ")", e);
		}
		return result;
	}
	...
	public DeathClaimDTO findDeathClaimById(String id) {
		...
		try {
			...
			throw new SystemException(e.getErrorCode(), "Faield to find a DeathClaim(ID : " + id + ")", e);
		}
		return result;
	}
	...
	public void updateMedicalClaimProposal(MedicalClaimProposal medicalClaimProposal, WorkFlowDTO workFlow) {
		try {
			if (workFlow.getWorkflowTask().equals(WorkflowTask.CLAIM_APPROVAL)) {
	...
	public void paymentMedicalClaimProposal(Payment payment, MedicalClaimProposalDTO medicalClaimProposalDTO) {
		try {
			...
			throw new SystemException(e.getErrorCode(), "Faield to Payment Medical Claim", e);
		}

	}

	private HospitalizedClaimDTO changeHospitalizedClaimDataToClaimDTO(HospitalizedClaim hospitalizedClaim) {
		HospitalizedClaimDTO hospitalizedClaimDTO = HospitalizedClaimFactory.createHospitalizedClaimDTO(hospitalizedClaim);
		return hospitalizedClaimDTO;
	}

	private MedicationClaimDTO changeMedicationClaimDataToClaimDTO(MedicationClaim medicationClaim) {
		MedicationClaimDTO medicationClaimDTO = MedicationClaimFactory.createMedicationClaimDTO(medicationClaim);
		return medicationClaimDTO;
	}

	private OperationClaimDTO changeOperationClaimDataToClaimDTO(OperationClaim operationClaim) {
		OperationClaimDTO operationClaimDTO = OperationClaimFactory.createOperationClaimDTO(operationClaim);
		return operationClaimDTO;
	}

	private DeathClaimDTO changeDeathClaimDataToClaimDTO(DeathClaim deathClaim) {
		DeathClaimDTO deathClaimDTO = DeathClaimFactory.createDeathClaimDTO(deathClaim);
