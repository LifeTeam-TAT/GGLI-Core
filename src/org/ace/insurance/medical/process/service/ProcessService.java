...
public class ProcessService extends BaseService implements IProcessService {
	...
	public List<Process> findAllProcess() {
		...
		try {
			...
			throw new SystemException(e.getErrorCode(), "Failed to find all of Process)", e);
		}
		return result;
	}

	@Transactional(propagation = Propagation.REQUIRED, readOnly = true)
	public List<Process001> findAll() {
		List<Process001> result = null;
		try {
			result = processDAO.findAllProcess();
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Failed to find all of Process)", e);
		}
		return result;
	...
	public Process findProcessById(String id) {
		Process result = null;
		try {
			result = processDAO.findById(id);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Failed to find a Process (ID : " + id + ")", e);
		}
		return result;
	}
}
