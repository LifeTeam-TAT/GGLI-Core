...
public class ProductProcessDAO extends BasicDAO implements IProductProcessDAO {
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED)
	public List<ProductProcess> findOldConfPP(String productId, String processId) throws DAOException {
		List<ProductProcess> result = null;
		try {
			Query query = em.createQuery(
					"SELECT p FROM ProductProcess p WHERE p.product.id = :productID and p.process.id = :processID and  p.activeStatus = org.ace.insurance.medical.productprocess.ActiveStatus.CONFIGURE ");
			...
			throw translate("Failed to find  configure of old ProductProcess by productId : " + productId + " and processId : " + processId, pe);
		}
		return result;
	}
