...
public class AgentCommissionDAO extends BasicDAO implements IAgentCommissionDAO {
	...
	public List<AC001> findAgentCommissionByAgent(AgentEnquiryCriteria agentEnquiryCriteria) throws DAOException {
		...
		try {
			...
			/* Executed query */
			Query query = em
					.createQuery("Select NEW org.ace.insurance.payment.AC001(mp.customer.name.firstName,mp.policyNo,ac.chalanNo,ac.commission) FROM AgentCommission ac JOIN Payment p on ac.chalanNo=p.chalanNo JOIN MedicalPolicy mp on p.referenceNo=mp.id Where ac.commissionStartDate is not null AND "
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void removeAgentcomissionByReceiptNo(String receiptNo) throws DAOException{
		try {
