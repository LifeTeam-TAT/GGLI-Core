...
public class ProductPremiumRateDAO extends BasicDAO implements IProductPremiumRateDAO {
	...
	public void update(ProductPremiumRate productPremiumRate) throws DAOException {
		try {
			...
			updateProcessLog(TableName.PRODUCT_PREMIUMRATE, productPremiumRate.getId());
			em.flush();
		} catch (PersistenceException pe) {
	...
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED)
	public Double findProductPremiumRate(Map<KeyFactor, String> keyfatorValueMap, Product product, SumInsuredType sumInsuredType) throws DAOException {
		Double result = null;
		try {
			boolean isSinglePremiumEndowmentLife;
			boolean isShortTermSinglePremiumCreditLife;
			boolean isSimpleLife;
			...
			if(isShortTermSinglePremiumCreditLife) 
				queryString.append("SELECT p FROM ProductPremiumRate p, ");
			else 
			...
			if (isShortTermSinglePremiumCreditLife) {
				List<ProductPremiumRate> premiumList = query.getResultList();
				
				if (premiumList != null && !premiumList.isEmpty()) {
					for (ProductPremiumRate premiumRateDTO : premiumList) {
						if(premiumRateDTO.getSumInsuredType().equals(sumInsuredType)){
							result = (Double) premiumRateDTO.getPremiumRate();
							break;
						}
					}
				}
			...
			switch (product.getPremiumRateType()) {
				case PERCENT_OF_SUMINSURED: {
					...
					break;
				case BASED_ON_BASE_SUMINSURED: {
					result = Utils.divide(Utils.getTwoDecimalPoint(result * sumInsuredAmount), product.getBaseSumInsured());
				}
					break;
				case USER_DEFINED_RATE:
					if (isSinglePremiumEndowmentLife) {
						result = Utils.getTwoDecimalPoint(result * sumInsuredAmount);
					}
					break;
				default:
					break;
		...
		try {
			...
			switch (product.getPremiumRateType()) {
				case PERCENT_OF_SUMINSURED: {
					...
					break;
				case BASED_ON_BASE_SUMINSURED: {
					result = (result * sumInsuredAmount) / product.getBaseSumInsured();
				}
					;
					break;
				case USER_DEFINED_RATE:
		...
		try {
			...
			for (Map.Entry<KeyFactor, String> entry : keyfatorValueMap.entrySet()) {
				...
				if (entry.getKey().getKeyFactorType().equals(KeyFactorType.FIXED)) {
					...
					queryString.append("prkf_" + i + ".keyFactor.value = :keyFactorName_" + i + " AND ");
					if (KeyFactorChecker.isSumInsured(entry.getKey())) {
						queryString.append("(prkf_" + i + ".from = :value_" + i + " AND prkf_" + i + ".to = :value_" + i + ")");
					} else {
						queryString.append("(prkf_" + i + ".from <= :value_" + i + " AND prkf_" + i + ".to >= :value_" + i + ")");
					}
