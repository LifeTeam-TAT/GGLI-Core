...
public class AgentCommissionDetailReportDAO extends BasicDAO implements IAgentCommissionDetailDAO {
	...
	private List<AgentCommissionDetailReport> findForLife(AgentCommissionDetailCriteria criteria) throws DAOException {
		...
		List<PolicyReferenceType> policyReferenceTypeList = new ArrayList<>();
		// policyReferenceTypeList.add(PolicyReferenceType.LIFE_POLICY);
		// policyReferenceTypeList.add(PolicyReferenceType.LIFE_BILL_COLLECTION);
		AgentCommissionDetailReport report = null;
		try {
			StringBuffer query = new StringBuffer();
			query.append("SELECT a FROM AgentCommissionDetailReportView a WHERE a.policyId IS NOT NULL AND a.insuranceType IN :referenceTypeList");
			...
			if (criteria.getInsuranceType() != null) {
				switch (criteria.getInsuranceType()) {
					case FARMER:
						policyReferenceTypeList.add(PolicyReferenceType.FARMER_POLICY);
						break;
					case LIFE:
						policyReferenceTypeList.add(PolicyReferenceType.LIFE_POLICY);
						policyReferenceTypeList.add(PolicyReferenceType.LIFE_BILL_COLLECTION);
					...
					case PERSONAL_ACCIDENT:
						policyReferenceTypeList.add(PolicyReferenceType.PA_POLICY);
						break;
					case PERSON_TRAVEL:
						policyReferenceTypeList.add(PolicyReferenceType.PERSON_TRAVEL_POLICY);
						break;
					case SHORT_ENDOWMENT_LIFE:
						policyReferenceTypeList.add(PolicyReferenceType.SHORT_ENDOWMENT_LIFE_POLICY);
						policyReferenceTypeList.add(PolicyReferenceType.SHORT_ENDOWMENT_LIFE_BILL_COLLECTION);
						break;
					case TRAVEL_INSURANCE:
						policyReferenceTypeList.add(PolicyReferenceType.TRAVEL_PROPOSAL);
					case MEDICAL:
					case MICRO_HEALTH:
					case CRITICAL_ILLNESS:
					case HEALTH:
						policyReferenceTypeList.addAll(PolicyReferenceType.getMedicalPolicyReference());
						break;
					default:
						break;
				}
			}
			q.setParameter("referenceTypeList", policyReferenceTypeList);
