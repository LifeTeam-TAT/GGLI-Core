...
public class FinancialReportDAO extends BasicDAO implements IFinancialReportDAO {
	...
	private void populateData(FinancialReport summaryReport, String reportItemType, List<Object> objList) {
		...
		for (Object obj : objList) {
			...
			long month = (Long) objArray[0];
			int year = (Integer) objArray[1];
			Integer m = (int) month;
	...
	private FinancialReport findMotorProposal(SummaryReportType type) {
		FinancialReport summaryReport = new FinancialReport();
		try {

		} catch (PersistenceException pe) {
			throw translate("Failed to find SummaryReport of Motor Proposal.", pe);
		}
		return summaryReport;
	}
	...
	private FinancialReport findAllAgentCommission(SummaryReportType type) {
		...
		try {
			...
			lifeQuery.setParameter("fromDate", startDate);
			lifeQuery.setParameter("toDate", endDate);
			lifeQuery.setParameter("lifePolicy", PolicyReferenceType.LIFE_POLICY);
			populateData(summaryReport, ReportItemType.LIFE, lifeQuery.getResultList());
	 ...
	 * emf.createEntityManager(); String motorQueryString =
	 * "SELECT COUNT(x.id), FUNC('MONTH', x.submittedDate) AS month, FUNC('YEAR', x.submittedDate) AS year FROM MotorProposal x WHERE :fromDate < x.submittedDate  AND x.submittedDate < :toDate GROUP BY year,month"
	 * ; Query q = em.createQuery(motorQueryString); List<Object> result =
	 * q.getResultList(); for (Object obj : result) { Object[] objArray =
	 * (Object[]) obj; System.out.println(objArray[0] + " : " + objArray[1] +
	 * " : " + objArray[2]); } em.close(); }
	 */
}
