...
public class WorkFlowReportDAO extends BasicDAO implements IWorkFlowReportDAO {
	...
	private class Queries {
		public static final String LIFE_WORKFLOW_STATUS = "SELECT COUNT(x.id), x.workflowTask FROM WorkFlow x WHERE x.referenceType = org.ace.insurance.common.WorkflowReferenceType.LIFE_PROPOSAL GROUP BY x.workflowTask";
	...
	public List<WorkFlowStatusReport> findStatusReport(WorkFlowStatusCriteria criteria) {
		...
		try {
			Query query = null;
			if (criteria.getWorkFlowType().equals(WorkFlowType.LIFE)) {
