		...
		try {
			...
			for (LifeProposal lifeProposal : list) {
				for (ProposalInsuredPerson person : lifeProposal.getProposalInsuredPersonList()) {
					...
					result.add(new GroupLifeProposalReport(lifeProposal.getProposalNo(), person.getInPersonGroupCodeNo(), agentNameAndCode, person.getFullName(), person
							.getFullAddress(), person.getProposedPremium(), person.getProposedPremium(), lifeProposal.getBranch().getName(), person
