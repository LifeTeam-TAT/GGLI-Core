...
public class LifeProposalReportDAO extends BasicDAO implements ILifeProposalReportDAO {
	...
	public List<LifeProposalReport> find(LifeProposalCriteria lifeProposalCriteria, List<String> productIdList) throws DAOException {
		...
		try {
			...
			if (null != lifeProposalCriteria.getEntity() && null == lifeProposalCriteria.getBranch()) {
				query.append(" AND lv.entitysId = :entityId");
			} else if (lifeProposalCriteria.getBranch() != null) {
				query.append(" AND lv.branchId = :branchId");
			}
			if (null != lifeProposalCriteria.getSalePoint()) {
				query.append(" AND lv.salePointId =:salePointId");
			}
			...
			if (null != lifeProposalCriteria.getEntity() && null == lifeProposalCriteria.getBranch()) {
				q.setParameter("entityId", lifeProposalCriteria.getEntity().getId());
			}
			if (lifeProposalCriteria.getBranch() != null) {
				q.setParameter("branchId", lifeProposalCriteria.getBranch().getId());
			}
			if (null != lifeProposalCriteria.getSalePoint()) {
				q.setParameter("salePointId", lifeProposalCriteria.getSalePoint().getId());
			}
			if (lifeProposalCriteria.getProduct() != null) {
	...
	public List<PersonalAccidentProposalReport> findPersonalAccidentProposal(LifeProposalCriteria lifeProposalCriteria) throws DAOException {
		...
		try {
			...
			if (lifeProposalCriteria.getEntity() != null && lifeProposalCriteria.getBranch() == null) {
				buffer.append(" AND p.entityId = :entityId");
			} else if (lifeProposalCriteria.getBranch() != null) {
				buffer.append(" AND p.branchId = :branchId");
			}
			if (lifeProposalCriteria.getSalePoint() != null) {
				buffer.append(" AND p.salePointId = :salePointId");
			}
			if (lifeProposalCriteria.getProduct() != null) {
			...
			if (lifeProposalCriteria.getEntity() != null && lifeProposalCriteria.getBranch() == null) {
				query.setParameter("entityId", lifeProposalCriteria.getEntity().getId());
			} else if (lifeProposalCriteria.getBranch() != null) {
				query.setParameter("branchId", lifeProposalCriteria.getBranch().getId());
			}
			if (lifeProposalCriteria.getSalePoint() != null) {
				query.setParameter("salePointId", lifeProposalCriteria.getSalePoint().getId());
			}
			if (lifeProposalCriteria.getProduct() != null) {
