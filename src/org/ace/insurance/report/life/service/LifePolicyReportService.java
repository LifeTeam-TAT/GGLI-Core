...
public class LifePolicyReportService implements ILifePolicyReportService {
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public List<NewShortTermEndowmentLifePolicyReport> findNewShortTermEndowLifePolicyReport(ShortTermEndowmentLifeReportCriteria criteria) {
		List<NewShortTermEndowmentLifePolicyReport> result = null;
		try {
			result = lifePolicyReportDAO.findNewShortTermEndowmentLifePolicyReport(criteria);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Failed to find ShortEndowLifePolicyReport by criteria.", e);
		}
		return result;
