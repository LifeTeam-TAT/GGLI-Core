...
public class CashierDAO extends BasicDAO implements ICashierDAO {
	...
	public void insert(Cashier cashier) throws DAOException {
		try {
			em.persist(cashier);
			insertProcessLog(TableName.CASHIER, cashier.getId());
			em.flush();
	...
	public void update(Cashier cashier) throws DAOException {
		try {
			em.merge(cashier);
			updateProcessLog(TableName.CASHIER, cashier.getId());
			em.flush();
		} catch (PersistenceException pe) {
	...
	public boolean checkExistingCashier(Cashier cashier) throws DAOException {
		...
		try {
			...
			query.append(" AND c.stateCode=:stateCode ");
			query.append(" AND c.townshipCode=:townshipCode ");
			...
			q.setParameter("stateCode", cashier.getStateCode());
			q.setParameter("townshipCode", cashier.getTownshipCode());
