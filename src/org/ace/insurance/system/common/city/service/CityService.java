...
public class CityService extends BaseService implements ICityService {
	...
	public void addNewCity(City city) {
		try {
			city.setPrefix(getPrefix(City.class));
			cityDAO.insert(city);
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<CITY001> findByCityCriteria(String criteria) {
		List<CITY001> result = null;
		try {
			result = cityDAO.findCityByCriteria(criteria);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find City by criteria " + criteria, e);
		}
		return result;
