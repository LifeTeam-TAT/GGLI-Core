...
public class CoinsuranceCompanyDAO extends BasicDAO implements ICoinsuranceCompanyDAO {
	...
	public void insert(CoinsuranceCompany coinsuranceCompany) throws DAOException {
		try {
			...
			insertProcessLog(TableName.COINSURANCE_COMPANY, coinsuranceCompany.getId());
			em.flush();
	...
	public void update(CoinsuranceCompany coinsuranceCompany) throws DAOException {
		try {
			...
			updateProcessLog(TableName.COINSURANCE_COMPANY, coinsuranceCompany.getId());
			em.flush();
		} catch (PersistenceException pe) {
	...
	public CoinsuranceCompany findById(String id) throws DAOException {
		...
		try {
			Query q = em.createNamedQuery("CoinsuranceCompany.findById");
			q.setParameter("id", id);
			ret = (CoinsuranceCompany) q.getSingleResult();
			em.flush();
		} catch (NoResultException pe) {
			return null;
		} catch (PersistenceException pe) {
			throw translate("Failed to find CoinsuranceCompany(CoinsuranceCompany = " + id + ")", pe);
		}
		return ret;
	}
	...
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED)
	public List<COINCOMPANY001> findAllCOINCOMPANY() throws DAOException {
		List<COINCOMPANY001> ret = null;
		try {
			Query q = em.createQuery("SELECT NEW " + COINCOMPANY001.class.getName() + " (c.id, c.name, c.description, c.address, c.contentInfo) FROM CoinsuranceCompany c ");
			ret = q.getResultList();
			em.flush();
		} catch (PersistenceException pe) {
			throw translate("Failed to find all of CoinsuranceCompany", pe);
		}
		return ret;
	}
}
