...
public class CurrencyService extends BaseService implements ICurrencyService {
	...
	public void addNewCurrency(Currency currency) throws SystemException {
		try {
			currency.setPrefix(getPrefix(Currency.class));
			...
			for (Coa chartOfAccount : coaList) {
				for (Branch branch : branchList) {
					...
					ccoa.setcUR(currency.getCurrencyCode());
	...
	public List<Currency001> findAllCurrency001() throws SystemException {
		List<Currency001> result = null;
		try {
			result = currencyDAO.findAllCurrency();
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find all of Currency)", e);
		}
		return result;
