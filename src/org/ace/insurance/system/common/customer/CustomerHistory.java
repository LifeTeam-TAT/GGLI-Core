...
import org.ace.insurance.medical.proposal.CustomerInfoStatusHistory;
...
@Entity
@Table(name = TableName.CUSTOMERHISTORY)
@TableGenerator(name = "CUSTOMER_HISRORY_GEN", table = "ID_GEN", pkColumnName = "GEN_NAME", valueColumnName = "GEN_VAL", pkColumnValue = "CUSTOMER_HISRORY_GEN", allocationSize = 10)
...
public class CustomerHistory implements Serializable {
	...
	@CollectionTable(name = "CUSTOMER_FAMILY_HISTORY", joinColumns = @JoinColumn(name = "CUSTOMERHISTORYID", referencedColumnName = "ID"))
	...
	@JoinColumn(name = "CUSTOMERHISTORYID", referencedColumnName = "ID")
	private List<CustomerInfoStatusHistory> customerStatushistory;
	...
	public CustomerHistory(Customer customer) {
		...
		this.activedDate = customer.getActivedDate();
		this.labourNo = customer.getLabourNo();
		this.birthMark = customer.getBirthMark();
		this.salary = customer.getSalary();
		this.closedPolicy = customer.getClosedPolicy();
		this.activePolicy = customer.getActivePolicy();
		this.bankAccountNo = customer.getBankAccountNo();
		...
		this.referenceMobileUserId = customer.getReferenceMobileUserId();
		this.gender = customer.getGender();
		this.idConditionType = customer.getIdConditionType();
		this.idType = customer.getIdType();
		this.maritalStatus = customer.getMaritalStatus();
		this.passportType = customer.getPassportType();
		this.officeAddress = customer.getOfficeAddress();
		this.permanentAddress = customer.getPermanentAddress();
		this.residentAddress = customer.getResidentAddress();
		this.contentInfo = customer.getContentInfo();
		this.name = customer.getName();
		if (customer.getFamilyInfo().size() > 0) {
			for (FamilyInfo info : customer.getFamilyInfo()) {
				familyInfohistory.add(new FamilyInfoHistory(info));
			}
		}
		this.branch = customer.getBranch();
		this.salePoint = customer.getSalePoint();
		this.qualification = customer.getQualification();
		this.bankBranch = customer.getBankBranch();
		...
		this.townshipCode = customer.getTownshipCode();
		if (customer.getCustomerStatusList().size() > 0) {
			for (CustomerInfoStatus customerinfostatus : customer.getCustomerStatusList()) {
				if (customerStatushistory == null) {
					customerStatushistory = new ArrayList<>();
				}
				customerStatushistory.add(new CustomerInfoStatusHistory(customerinfostatus));
			}
		}
		this.commonCreateAndUpateMarks = customer.getCommonCreateAndUpateMarks();
	}

	@Id
	@GeneratedValue(strategy = GenerationType.TABLE, generator = "CUSTOMER_HISRORY_GEN")
	...
	public List<FamilyInfoHistory> getFamilyInfo() {
		if (this.familyInfohistory == null) {
			this.familyInfohistory = new ArrayList<FamilyInfoHistory>();
		}
		return this.familyInfohistory;
	}

	public void setFamilyInfo(List<FamilyInfoHistory> familyInfo) {
		this.familyInfohistory = familyInfo;
	}
	...
	public List<CustomerInfoStatusHistory> getCustomerStatusList() {
		if (customerStatushistory == null) {
			customerStatushistory = new ArrayList<CustomerInfoStatusHistory>();
		}
		return customerStatushistory;
	}

	public void setCustomerStatusList(List<CustomerInfoStatusHistory> customerStatusList) {
		this.customerStatushistory = customerStatusList;
	}

	public void addCustomerInfoStatus(CustomerInfoStatusHistory customerInfoStatus) {
		if (customerStatushistory == null) {
			customerStatushistory = new ArrayList<CustomerInfoStatusHistory>();
		}
		if (!customerStatushistory.contains(customerInfoStatus)) {
			customerStatushistory.add(customerInfoStatus);
		}
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((customerStatushistory == null) ? 0 : customerStatushistory.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		CustomerHistory other = (CustomerHistory) obj;
		...
		if (country == null) {
			...
			return false;
		if (customerStatushistory == null) {
			if (other.customerStatushistory != null)
				return false;
		} else if (!customerStatushistory.equals(other.customerStatushistory))
			return false;
		if (dateOfBirth == null) {
