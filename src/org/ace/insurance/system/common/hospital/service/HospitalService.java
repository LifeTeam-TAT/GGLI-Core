...
public class HospitalService extends BaseService implements IHospitalService {
	...
	public void addNewHospital(Hospital hospital) {
		try {
			hospital.setPrefix(getPrefix(Hospital.class));
	...
	@Transactional(propagation = Propagation.REQUIRED, readOnly = true)
	public List<Hospital001> findAllHospital001() {
		List<Hospital001> result = null;
		try {
			result = hospitalDAO.findAllHospital();
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find all of Hospital)", e);
		}
		return result;
	...
	@Transactional(propagation = Propagation.REQUIRED, readOnly = true)
	public List<Hospital001> findByCriteria001(String criteria) {
		List<Hospital001> result = null;
		try {
			result = hospitalDAO.findByCriteria001(criteria);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find Hospital by criteria " + criteria, e);
		}
		return result;
