...
public class IndustryService extends BaseService implements IIndustryService {
	...
	public void addNewIndustry(Industry Industry) {
		try {
			Industry.setPrefix(getPrefix(Industry.class));
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadExistIndustry(Industry industry) {
		try {
			return industryDAO.isAlreadExistIndustry(industry);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find Industry by Name ");
		}
	}
