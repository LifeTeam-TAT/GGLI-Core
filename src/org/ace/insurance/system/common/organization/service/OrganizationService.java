...
public class OrganizationService extends BaseService implements IOrganizationService {
	...
	public void addNewOrganization(Organization organization) {
		try {
			organization.setPrefix(getPrefix(Organization.class));
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExitOrganization(Organization organization) {
		try {
			return organizationDAO.isAlreadyExitOrganization(organization);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Failed to find Existing Organization Name", e);
		}
