...
public class QualificationService extends BaseService implements IQualificationService {
	...
	public void addNewQualification(Qualification qualification) {
		try {
			qualification.setPrefix(getPrefix(Qualification.class));
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExist(Qualification qualification) {
		try {
			return qualificationDAO.isAlreadyExist(qualification);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find Qualification by Name", e);
		}
