...
public class RelationShipService extends BaseService implements IRelationShipService {
	...
	public void addNewRelationShip(RelationShip relationShip) {
		try {
			relationShip.setPrefix(getPrefix(RelationShip.class));
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<RELATION001> findRelationShip() {
		List<RELATION001> result = null;
		try {
			result = relationShipDAO.findRelationship();
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find all of RelationShip)", e);
		}
		return result;
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExistRelationShip(RelationShip relationShip) {
		try {
			return relationShipDAO.isAlreadyExistRelationShip(relationShip);
		} catch (DAOException e) {
