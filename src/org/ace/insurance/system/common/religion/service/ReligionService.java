...
public class ReligionService extends BaseService implements IReligionService {
	...
	public void addNewReligion(Religion religion) {
		try {
			religion.setPrefix(getPrefix(Religion.class));
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExistReligion(Religion religion) {
		try {
			return religionDAO.isAlreadyExistReligion(religion);
		} catch (DAOException e) {
			throw new SystemException(e.getErrorCode(), "Faield to find Religion by Name", e);
		}
	}
