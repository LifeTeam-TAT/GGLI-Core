...
public class StateCodeDAO extends BasicDAO implements IStateCodeDAO {
	...
	public void insert(StateCode stateCode) throws DAOException {
		try {
			if (!isAlreadyExistStateCode(stateCode)) {
				em.persist(stateCode);
				em.flush();
			} else {
				throw new SystemException(null, stateCode.getName() + " is already exist.");
			}
	...
	public StateCode findByCodeNo(String stateCodeNo, String townshipCodeNo) throws DAOException {
		StateCode result = null;
		try {
			Query query = em.createQuery("SELECT s FROM TownshipCode t JOIN t.stateCode s  WHERE s.codeNo = :codeNo AND t.townshipcodeno = :townshipCodeNo");
			query.setParameter("codeNo", stateCodeNo);
			query.setParameter("townshipCodeNo", townshipCodeNo);
			result = (StateCode) query.getSingleResult();
			em.flush();
	...
	public List<StateCode> findByCriteria(String criteria) throws DAOException {
		List<StateCode> result = null;
		try {
			Query q = em.createQuery("Select s from StateCode s where s.name Like '" + criteria + "%'");
			result = q.getResultList();
			em.flush();
		} catch (PersistenceException pe) {
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExistStateCode(StateCode stateCode) throws DAOException {
		boolean exist = false;
		String stateCodeName = stateCode.getName().replaceAll("\\s+", "");
		try {
			StringBuffer buffer = new StringBuffer();
			Query query = null;

			buffer = new StringBuffer("SELECT CASE WHEN (COUNT(c.name) > 0) THEN TRUE ELSE FALSE END FROM StateCode c ");
			buffer.append(" WHERE LOWER(FUNCTION('REPLACE',c.name,' ','')) = :name ");
			buffer.append(stateCode.getId() != null ? "AND c.id != :id" : "");
			query = em.createQuery(buffer.toString());
			if (stateCode.getId() != null)
				query.setParameter("id", stateCode.getId());
			query.setParameter("name", stateCodeName.toLowerCase());
