...
import org.ace.insurance.system.common.townshipCode.persistence.interfaces.ITownshipCodeDAO;
...
public class TownshipCodeDAO extends BasicDAO implements ITownshipCodeDAO {
	...
	public void insert(TownshipCode townshipCode) throws DAOException {
		try {
			if (!isAlreadyExistTownshipCode(townshipCode)) {
				em.persist(townshipCode);
				em.flush();
			} else {
				throw new SystemException(null, townshipCode.getName() + " is already exist.");
			}
	...
	public TownshipCode findByCodeNo(String tspCodeNo, String stateCodeNo) throws DAOException {
		TownshipCode result = null;

		try {
			Query query = em.createQuery("SELECT t FROM TownshipCode t JOIN t.stateCode s WHERE t.townshipcodeno = :codeNo AND s.codeNo = :stateCodeNo");
			query.setParameter("codeNo", tspCodeNo);
			query.setParameter("stateCodeNo", stateCodeNo);
			result = (TownshipCode) query.getSingleResult();
			em.flush();
	...
	public List<TownshipCode> findByCriteria(String criteria) throws DAOException {
		...
		try {
			...
			Query q = em.createQuery("Select t from TownshipCode t where t.name Like '" + criteria + "%'");
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<TownshipCode> findByStateCode(String stateCodeId) throws DAOException {
		List<TownshipCode> result = null;
		try {
			Query q = em.createQuery("Select t from TownshipCode t where t.stateCode.id =:stateCodeId ORDER BY t.name");
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<TownshipCode> findByStateCodeNo(String stateCode) {
		List<TownshipCode> result = null;
		try {
			Query q = em.createQuery("Select t from TownshipCode t where t.stateCode.codeNo =:stateCode ORDER BY t.name");
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public boolean isAlreadyExistTownshipCode(TownshipCode townshipCode) throws DAOException {
		boolean exist = false;
		String townshipCodeName = townshipCode.getName().replaceAll("\\s+", "");
		try {
			StringBuffer buffer = new StringBuffer();
			Query query = null;

			buffer = new StringBuffer("SELECT CASE WHEN (COUNT(c.name) > 0) THEN TRUE ELSE FALSE END FROM TownshipCode c ");
			buffer.append(" WHERE LOWER(FUNCTION('REPLACE',c.name,' ','')) = :name ");
			buffer.append(townshipCode.getId() != null ? "AND c.id != :id" : "");
			query = em.createQuery(buffer.toString());
			if (townshipCode.getId() != null)
				query.setParameter("id", townshipCode.getId());
			query.setParameter("name", townshipCodeName.toLowerCase());
