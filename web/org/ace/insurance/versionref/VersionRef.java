...
@TableGenerator(name = "WSVERREF_GEN", table = "ID_GEN", pkColumnName = "GEN_NAME",
				valueColumnName = "GEN_VAL", pkColumnValue = "WSVERREF_GEN", allocationSize = 10)
@NamedQueries(value = {
	@NamedQuery(name = "VersionRef.findAll",
				query = "SELECT v FROM VersionRef v"),
	@NamedQuery(
				name = "VersionRef.findAllByGroup",
				query = "SELECT v FROM VersionRef v WHERE v.group = :group ORDER BY v.versionNo DESC"),
	@NamedQuery(
	...
	@NamedQuery(
			...
			query = "SELECT v FROM VersionRef v " +
					"WHERE v.versionNo > :versionNo AND v.group = :group AND v.entityName = :entityName " +
					"ORDER BY v.entityName"),
	@NamedQuery(
