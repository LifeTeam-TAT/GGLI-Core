...
@ViewScoped
@ManagedBean(name = "StudentLifePaymentSkipActionBean")
public class StudentLifePaymentSkipActionBean extends BaseBean implements Serializable {
	...
	private PolicyReferenceType studentLifeReferenceType;
	...
	public void init() {
		...
		isRequired = false;
		skipTLF = true;
		studentLifeReferenceType = PolicyReferenceType.STUDENT_LIFE_POLICY_BILL_COLLECTION;
	}
	...
	public PolicyReferenceType[] getReferenceTypes() {
		return new PolicyReferenceType[] { PolicyReferenceType.STUDENT_LIFE_POLICY_BILL_COLLECTION };
	}
	...
	public void search() {
		...
		paymentList = paymentService.findPaymentByRecipNo(new ArrayList<String>(receiptSet), studentLifeReferenceType, false);
	}
	...
	public String paymentSkipTLF() {
		...
		try {
			if (validation()) {
				if (skipTLF) {
					paymentDelegateService.paymentSkipTLF(paymentList, studentLifeReferenceType, responsiblePerson, remark, user.getBranch(), salePoint);
				} else {
					paymentDelegateService.payment(paymentList, referenceType, responsiblePerson, remark, user.getBranch(), salePoint);
				}
				ExternalContext extContext = getFacesContext().getExternalContext();
	...
	public PolicyReferenceType getStudentLifeReferenceType() {
		return studentLifeReferenceType;
	}

	public void setStudentLifeReferenceType(PolicyReferenceType studentLifeReferenceType) {
		this.studentLifeReferenceType = studentLifeReferenceType;
	}
