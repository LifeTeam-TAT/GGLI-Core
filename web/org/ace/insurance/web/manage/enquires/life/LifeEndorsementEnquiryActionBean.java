...
@ViewScoped
@ManagedBean(name = "LifeEndorsementEnquiryActionBean")
public class LifeEndorsementEnquiryActionBean extends BaseBean implements Serializable {
	...
	public void showDetailLifeProposal(LPL001 lifeProposalDTO) {
		...
		if (lifeProposal.getLifePolicy() != null) {
			if (isFinishedConfirm(lifeProposalDTO.getId())) {
				this.lifeEndorseInfo = lifeEndorsementService.findByEndorsePolicyReferenceNo(lifeProposal.getLifePolicy().getId());
			} else {
				this.lifeEndorseInfo = lifeEndorsementService.findBySourcePolicyReferenceNo(lifeProposal.getLifePolicy().getId());
			}
		}

	}

	private boolean isFinishedConfirm(String referenceNo) {
		WorkFlow wf = workFlowService.findWorkFlowByRefNo(referenceNo);
		if (wf == null || wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_PAYMENT) || wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_ISSUING)) {
			return true;
		}
		return false;
	}
	...
	public void resetCriteria() {
		...
		criteria.setNumber("");
		criteria.setEndorsement(true);
		criteria.setProposalType(ProposalType.ENDORSEMENT);
		lifeProposalList = lifeProposalService.findLifeProposalByEnquiryCriteria(criteria, productList);
	}
	...
	public String editLifeProposal(LPL001 lifeProposalDTO) {
		...
		if (allowToEdit(lifeProposal.getId())) {
			...
			putParam("proposalType", ProposalType.ENDORSEMENT);
	...
	public String printIssuePolicy(LPL001 lifeProposalDTO) {
		LifeProposal lifeProposal = lifeProposalService.findLifeProposalById(lifeProposalDTO.getId());
		if (allowToPrint(lifeProposal, WorkflowTask.ENDORSEMENT_ISSUING)) {
			outjectLifeProposal(lifeProposal);
			return "printIssuePolicy";
		} else {
			this.message = getMessage(MessageId.WORKFLOW_INFORMATION_MESSAGE, WorkflowTask.ENDORSEMENT_PAYMENT.getLabel().toLowerCase());
		}
		return null;
	}
	...
	public void generateLifePolicyLedger(LPL001 lifeProposalDTO) {
		if (allowToPrint(lifeProposalService.findLifeProposalById(lifeProposalDTO.getId()), WorkflowTask.ENDORSEMENT_ISSUING, WorkflowTask.ENDORSEMENT_PAYMENT)) {
			...
			if (checkGroupLife(lifePolicy)) {
				...
				pdfDirPath = "/pdf-report/" + reportName + "/" + currentTime + "/";
			...
			this.message = getMessage(MessageId.WORKFLOW_INFORMATION_MESSAGE, WorkflowTask.ENDORSEMENT_CONFIRMATION.getLabel().toLowerCase());
		}
	}

	public void handleClosePolicyLedger(CloseEvent event) {
		try {
	...
	public void generateLifeSanction(LPL001 lifeProposalDTO) {
		LifeProposal lifeProposal = lifeProposalService.findLifeProposalById(lifeProposalDTO.getId());
		if (allowToPrint(lifeProposalService.findLifeProposalById(lifeProposalDTO.getId()), WorkflowTask.ENDORSEMENT_ISSUING, WorkflowTask.ENDORSEMENT_PAYMENT,
				WorkflowTask.ENDORSEMENT_CONFIRMATION, WorkflowTask.ENDORSEMENT_INFORM, WorkflowTask.ENDORSEMENT_APPROVAL)) {
			if (isSinglePrint()) {
				String reportName = "PublicLifeSanction";
				pdfDirPath = "/pdf-report/" + reportName + "/" + currentTime + "/";
				dirPath = getWebRootPath() + pdfDirPath;
				fileName = reportName + ".pdf";
				DocumentBuilder.generatePublicLifeSanction(lifeProposal, dirPath, fileName);
			} else if (isGroupPrint()) {
				String reportName = "GroupLifeSanction";
				pdfDirPath = "/pdf-report/" + reportName + "/" + currentTime + "/";
				dirPath = getWebRootPath() + pdfDirPath;
				fileName = reportName + ".pdf";
				DocumentBuilder.generateGroupLIfeSanction(lifeProposal, lifeProposal.getCustomerName(), dirPath, fileName);
			}
			PrimeFaces.current().executeScript("PF('documentPrintDailog').show()");
		} else {
			this.message = getMessage(MessageId.WORKFLOW_INFORMATION_MESSAGE, WorkflowTask.ENDORSEMENT_CONFIRMATION.getLabel().toLowerCase());
	...
	private boolean allowToEdit(String refNo) {
		...
		if (wf == null) {
			...
			if (wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_UNDERWRITING) || wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_SURVEY)
					|| wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_APPROVAL) || wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_INFORM)
					|| wf.getWorkflowTask().equals(WorkflowTask.ENDORSEMENT_CONFIRMATION)) {
