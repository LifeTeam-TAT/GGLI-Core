 ...
 */
@ViewScoped
@ManagedBean(name = "StudentLifePaymentSkipBillCollectionActionBean")
public class StudentLifePaymentSkipBillCollectionActionBean extends BaseBean implements Serializable {
	...
	public void reset() {
		...
		policyCriteria.setReferenceType(ReferenceType.STUDENT_LIFE_POLICY);
		policyCriteria.setPolicyCriteria(PolicyCriteriaItems.POLICYNO);
		channelValue = null;
		isAccBank = false;
		isCheque = false;
		isBank = false;
	...
	public void confirmSkipTLF() {
		...
		try {
			...
			for (BillCollectionDTO billCollection : selectedDTOList) {
				...
				payment.setToTerm(billCollection.getLastPaymentTerm() + billCollection.getPaymentTimes());
				payment.setPaymentType(billCollection.getPaymentType());
				payment.setConfirmDate(confirmDate);
	...
	public ReferenceType[] getReferenceTypeSelectedItemList() {
		return new ReferenceType[] { ReferenceType.STUDENT_LIFE_POLICY };
	}
