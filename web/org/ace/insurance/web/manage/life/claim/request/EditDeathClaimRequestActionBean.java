 ...
 */
@ViewScoped
@ManagedBean(name = "EditLifeDeathClaimRequestActionBean")
public class EditDeathClaimRequestActionBean extends BaseBean implements Serializable {
	...
	private final String FROM_CUSTOMER_SERVICE = "fromCustomerService";
	...
	public void init() {
		...
		if (transition.equals(FROM_CUSTOMER_SERVICE)) {
		...
		WorkFlow workFlow = workFlowDTOService.findWorkFlowByRefNo(lifeClaim.getClaimRequestId(), WorkflowTask.UNDERWRITING);
	...
	public void handleClaimAttachment(FileUploadEvent event) {
		...
		String fileName = Long.toString(System.currentTimeMillis()) + "_" + uploadedFile.getFileName();
	...
	public String onFlowProcess(FlowEvent event) {
		...
		if ("claimInfo".equals(event.getOldStep())) {
			...
			if (deathClaimDTO.getSaleman() == null) {
				addErrorMessage(formID + ":saleMan", MessageId.REQUIRED_SALEMAN);
				valid = false;
			}
	...
	public String submitClaimInfo() {
		...
		try {
			...
			WorkFlowDTO workFlowDTO = new WorkFlowDTO(lifeClaim.getClaimRequestId(), remark, workflowTask, WorkflowReferenceType.LIFE_CLAIM, user, responsiblePerson);
			lifeClaimService.approveLifeClaim(getLifeClaim(), workFlowDTO);
			/* file upload */
	...
	public DeathClaimDTO populateClaimInfoDTO() {
		...
		claimInfoDTO.setSaleman(lifeClaim.getSaleman());
	...
	private LifeClaim getLifeClaim() {
		...
		claim.setSaleman(deathClaimDTO.getSaleman());
	...
	public void selectUser() {
		selectUser(WorkflowTask.CLAIM_APPROVAL, WorkFlowType.LIFE);
	}
