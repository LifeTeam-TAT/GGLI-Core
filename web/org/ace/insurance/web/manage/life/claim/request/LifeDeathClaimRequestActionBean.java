...
public class LifeDeathClaimRequestActionBean extends BaseBean implements Serializable {
	...
	public void updatePolicyInsuredPerson() {
		...
		if (valid) {
			PrimeFaces.current().executeScript("PF('updatedeathPersonInfoDialog').hide()");
		}
	}
	...
	public String submitClaimInfo() {
		...
		try {
			...
			if (pending) {
				workflowTask = WorkflowTask.CLAIM_REQUEST;
			}
			if (fromDashBoard) {
				workFlowDTO = new WorkFlowDTO(lifeClaim.getClaimRequestId(), remark, workflowTask, WorkflowReferenceType.LIFE_CLAIM, user, responsiblePerson);
				lifeClaimService.approveLifeClaim(getLifeClaim(), workFlowDTO);
			} else {
				workFlowDTO = new WorkFlowDTO("", remark, workflowTask, WorkflowReferenceType.LIFE_CLAIM, user, responsiblePerson);
				lifeClaimService.addNewLifeClaim(getLifeClaim(), workFlowDTO);
			}
			/* file upload */
	...
	public void returnSaleMan(SelectEvent event) {
		...
		deathClaimDTO.setSaleman(saleMan);
	}
