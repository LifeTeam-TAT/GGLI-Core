...
public class LifeDisabilityClaimRequestActionBean extends BaseBean implements Serializable {
	...
	public void init() {
		...
		if (claim == null) {
			...
			disabilityClaimDTO.setSaleMan(disabilityClaim.getSaleman());
		...
		WorkFlow workFlow = workFlowDTOService.findWorkFlowByRefNo(claim.getClaimRequestId(), WorkflowTask.UNDERWRITING);
	...
	public String submitClaimInfo() {
		...
		if (responsiblePerson == null) {
			...
			if (!isUpdate) {
				workFlowDTO = new WorkFlowDTO("", remark, workflowTask, WorkflowReferenceType.LIFE_DIS_CLAIM, user, responsiblePerson);
				claimService.addNewLifeClaim(getLifeClaim(), workFlowDTO);
			} else {
				workFlowDTO = new WorkFlowDTO(claim.getClaimRequestId(), remark, workflowTask, WorkflowReferenceType.LIFE_DIS_CLAIM, user, responsiblePerson);
	...
	private LifeClaim getLifeClaim() {
		...
		disClaim.setSaleman(disabilityClaimDTO.getSaleMan());
	...
	public void returnSaleMan(SelectEvent event) {
		...
		disabilityClaimDTO.setSaleMan(saleMan);
	}
	...
	public void setLifePolicy() {
		LFP001 lplcDTO001 = (LFP001) getParam("LplcDTO001");
		...
		disabilityClaimDTO.setSaleMan(lifePolicy.getSaleMan());
