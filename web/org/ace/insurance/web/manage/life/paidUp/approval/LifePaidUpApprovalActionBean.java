...
public class LifePaidUpApprovalActionBean extends BaseBean implements Serializable {
	...
	public String addPaidUpApproval() {
		...
		try {
			if (paidUpProposal.getLifePolicy().getInsuredPersonInfo().get(0).getProduct().getId().equals(KeyFactorChecker.getPublicLifeID())) {
				workFlowDTO = new WorkFlowDTO(paidUpProposal.getId(), remark, WorkflowTask.INFORM, WorkflowReferenceType.LIFE_PAIDUP_PROPOSAL, user, responsiblePerson);
			} else {
				workFlowDTO = new WorkFlowDTO(paidUpProposal.getId(), remark, WorkflowTask.INFORM, WorkflowReferenceType.SHORTTERM_LIFE_PAIDUP_PROPOSAL, user, responsiblePerson);
			}
			workFlowDTO = new WorkFlowDTO(paidUpProposal.getId(), remark, WorkflowTask.INFORM, WorkflowReferenceType.LIFE_PAIDUP_PROPOSAL, user, responsiblePerson);
	...
	public void selectUser() {
		selectUser(WorkflowTask.INFORM, WorkFlowType.LIFE_PAIDUP);
	}
