...
public class LifePaidUpInformActionBean extends BaseBean implements Serializable {
	...
	public void informLifePaidUp() {
		...
		try {
			...
			if (paidUpProposal.getLifePolicy().getInsuredPersonInfo().get(0).getProduct().getId().equals(KeyFactorChecker.getPublicLifeID())) {
				workFlowDTO = new WorkFlowDTO(paidUpProposal.getId(), remark, WorkflowTask.CONFIRMATION, WorkflowReferenceType.LIFESURRENDER_PROPOSAL, user, responsiblePerson);
			} else {
				workFlowDTO = new WorkFlowDTO(paidUpProposal.getId(), remark, WorkflowTask.CONFIRMATION, WorkflowReferenceType.SHORTTERM_LIFE_PAIDUP_PROPOSAL, user,
						responsiblePerson);
			}
			lifePaidUpProposalService.informLifePaidUpProposal(paidUpProposal, workFlowDTO, claimAcceptedInfo);
			addInfoMessage(null, MessageId.INFORM_PROCESS_SUCCESS, paidUpProposal.getProposalNo());
			printFlag = true;
	...
	public void selectUser() {
		...
		selectUser(WorkflowTask.CONFIRMATION, WorkFlowType.LIFE_PAIDUP);
	}
