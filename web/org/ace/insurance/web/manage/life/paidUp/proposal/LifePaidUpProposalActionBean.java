...
public class LifePaidUpProposalActionBean extends BaseBean implements Serializable {
	...
	public void createNewLifePaidUpProposal() {
		...
		lifePaidUpProposal.setActivedPolicyStartDate(new Date());
		lifePaidUpProposal.setActivedPolicyEndDate(new Date());
	...
	public void returnLifePolicy(SelectEvent event) {
		...
		if (lifePolicy.getInsuredPersonInfo().get(0).getProduct().getId().equals(KeyFactorChecker.getPublicLifeID())) {
			isLife = true;
			claimProduct = claimProductService.findClaimProductById(KeyFactorChecker.getLifePaidUpProductId());
		} else {
			isLife = false;
			claimProduct = claimProductService.findClaimProductById(KeyFactorChecker.getShortTermPaidUpProductId());
		}
	...
	public String addNewLifePaidUpProposal() {
		...
		try {
			if (!isLife) {
				workFlowDTO = new WorkFlowDTO(null, remark, WorkflowTask.APPROVAL, WorkflowReferenceType.SHORTTERM_LIFE_PAIDUP_PROPOSAL, user, responsiblePerson);

			} else {
				workFlowDTO = new WorkFlowDTO(null, remark, WorkflowTask.APPROVAL, WorkflowReferenceType.LIFE_PAIDUP_PROPOSAL, user, responsiblePerson);
			}
	...
	public void loadPaymentConfig() {
		...
		if (paymentType > 0) {
			realAmount = basicPremium * ((paymentYear * 12) / paymentType);
		}
	...
	public void returnSaleMan(SelectEvent event) {
		...
		lifePaidUpProposal.setSaleMan(saleMan);
	}

	public void selectUser() {
		WorkFlowType workFlowType = isLife ? WorkFlowType.LIFE_PAIDUP : WorkFlowType.SHORTTERM_LIFE_PAIDUP;
		selectUser(WorkflowTask.APPROVAL, workFlowType);
	}
