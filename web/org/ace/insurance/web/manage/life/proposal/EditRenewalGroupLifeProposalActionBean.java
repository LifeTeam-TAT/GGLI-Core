...
@ViewScoped
@ManagedBean(name = "EditRenewalGroupLifeProposalActionBean")
public class EditRenewalGroupLifeProposalActionBean extends BaseBean implements Serializable {
	...
	private boolean validInsuredPerson(InsuredPersonInfoDTO insuredPersonInfoDTO) {
		...
		// if(insuredPersonInfoDTO.getBeneficiariesInfoDTO1List().isEmpty()) {
		// addErrorMessage(formID + ":beneficiariesInfoTablePanel",
		// MessageId.REQUIRED_BENEFICIARY_PERSON);
		// valid = false;
		// } else {
		// float totalPercent = 0.0f;
		// for(BeneficiariesInfoDTO bfDTO :
		// insuredPersonInfoDTO.getBeneficiariesInfoDTOList()) {
		...
		// }
		/*
		 * // for(BeneficiariesInfoDTO benfInfoDTO :
		 * insuredPersonInfoDTO.getBeneficiariesInfoDTOList()) { //
		 * if(!benfInfoDTO.isValidBeneficiaries()) { // addErrorMessage(formID +
		 * ":beneficiariesInfoTablePanel",
		 * MessageId.INVALID_BENEFICIARY_PERSON); // valid = false;break; // }
		 * // }
		 */
	...
	public String updateLifeProposal() {
		...
		try {
			...
			workflowTask = surveyAgain ? WorkflowTask.RENEWAL_SURVEY : WorkflowTask.RENEWAL_APPROVAL;
			...
			lifeProposalService.updateLifeProposal(lifeProposal, workFlowDTO, null);
	...
	public void returnBeneficaryTownShip(SelectEvent event) {
		Township townShip = (Township) event.getObject();
