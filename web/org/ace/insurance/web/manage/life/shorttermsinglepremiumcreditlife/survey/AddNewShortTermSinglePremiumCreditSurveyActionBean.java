package org.ace.insurance.web.manage.life.shorttermsinglepremiumcreditlife.survey;
...
@ViewScoped
@ManagedBean(name = "AddNewShortTermSinglePremiumCreditSurveyActionBean")
public class AddNewShortTermSinglePremiumCreditSurveyActionBean extends BaseBean implements Serializable {
	...
	private final String PROPOSAL_DIR = "/upload/STSPClife-proposal/";
	...
	public void init() {
		...
		if (lifeProposal.getLifePolicy() != null) {
			...
			for (LifeProposalAttachment attach : lifeProposal.getAttachmentList()) {
				...
				filePath = filePath.replaceAll("/upload/STSPClife-proposal/", temporyDir);
	...
	public void copyAttachment(LifeProposal lifeProposal) {
		...
		if (lifeProposal.getAttachmentList().size() > 0) {
			...
			for (LifeProposalAttachment attach : lifeProposal.getAttachmentList()) {
				...
				fPath = fPath.replaceAll("/upload/STSPClife-proposal/", temporyDir);
	...
	private boolean validateSurveyQuestionsAnswer() {
		...
		if (getSurveyQuestionAnswerDTOList().size() <= 0) {
			String formID = "surveyEntryForm";
	...
	public String addNewSurvey() {
		...
		try {
			if (validateAttachement() && validateSurveyQuestionsAnswer()) {
				...
				WorkflowReferenceType referenceType = WorkflowReferenceType.SHORT_TERM_SINGLE_PREMIUM_CREDIT_LIFE_PROPOSAL;
	...
	public void generateReport() {
		...
		DocumentBuilder.generateShortTermSinglePremiumCreditLifeSanction(proposal, dirPath, fileName);
	...
	public void selectUser() {
		...
		WorkFlowType workFlowType = WorkFlowType.SHORT_TERM_SINGLE_PREMIUM_CREDIT_LIFE;
		selectUser(workflowTask, workFlowType);
	}

	/* SurveyQuestionAnswer in Short term Single Premium Credit Life */
			...
			if (pip.getId().equals(insuredPerson.getId())) {
				if (pip.getSurveyQuestionAnswerList() != null && !pip.getSurveyQuestionAnswerList().isEmpty()) {
					for (SurveyQuestionAnswer sqanswer : pip.getSurveyQuestionAnswerList()) {
						...
						surveyAnswerDTO.setSurveyType(SurveyType.SHORT_TERM_SINGLE_PERMIUM_CREDIT_LIFE_SURVEY);
					...
					List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService
							.findPProQueByPPIdByOptionTypeForSTSPCL(KeyFactorChecker.getShortTermSinglePremiumCreditLifeID(), ProductProcessIDConfig.getProposalProcessId(), dto);
					for (ProductProcessQuestionLink ppQuesLink : ppQueLinkList) {
						...
						surveyAnswer.setSurveyType(SurveyType.SHORT_TERM_SINGLE_PERMIUM_CREDIT_LIFE_SURVEY);
