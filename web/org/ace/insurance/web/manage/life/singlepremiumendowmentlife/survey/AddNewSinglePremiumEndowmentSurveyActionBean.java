package org.ace.insurance.web.manage.life.singlepremiumendowmentlife.survey;
...
@ViewScoped
@ManagedBean(name = "AddNewSinglePremiumEndowmentSurveyActionBean")
public class AddNewSinglePremiumEndowmentSurveyActionBean extends BaseBean implements Serializable {
	...
	private final String PROPOSAL_DIR = "/upload/SPElife-proposal/";
	...
	public void init() {
		...
		if (lifeProposal.getLifePolicy() != null) {
			...
			for (LifeProposalAttachment attach : lifeProposal.getAttachmentList()) {
				...
				filePath = filePath.replaceAll("/upload/SPElife-proposal/", temporyDir);
	...
	public void copyAttachment(LifeProposal lifeProposal) {
		...
		if (lifeProposal.getAttachmentList().size() > 0) {
			...
			for (LifeProposalAttachment attach : lifeProposal.getAttachmentList()) {
				...
				fPath = fPath.replaceAll("/upload/SPElife-proposal/", temporyDir);
	...
	public void generateReport() {
		...
		fileName = "SinglePremiumEndowmentLife_" + customerName + "_Sanction" + ".pdf";
		LifeProposal proposal = lifeSurvey.getLifeProposal();
		DocumentBuilder.generateSinglePremiumEndowmentLifeSanction(proposal, dirPath, fileName);
	...
	public void selectUser() {
		WorkflowTask workflowTask = isEndorse ? WorkflowTask.ENDORSEMENT_APPROVAL : WorkflowTask.APPROVAL;
		WorkFlowType workFlowType = WorkFlowType.SINGLE_PREMIUM_ENDOWMENT_LIFE;
		selectUser(workflowTask, workFlowType);
	}

	/* SurveyQuestionAnswer in Single Premium Endowment Life */
			...
			if (pip.getId().equals(insuredPerson.getId())) {
				if (pip.getSurveyQuestionAnswerList() != null && !pip.getSurveyQuestionAnswerList().isEmpty()) {
					for (SurveyQuestionAnswer sqanswer : pip.getSurveyQuestionAnswerList()) {
						...
						surveyAnswerDTO.setSurveyType(SurveyType.SINGLE_PERMIUM_ENDOWMENT_LIFE_SURVEY);
						surveyQuestionAnswerDTOList.add(surveyAnswerDTO);
					}
				} else {

					ProductProcessCriteriaDTO dto = new ProductProcessCriteriaDTO();
					List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService.findPProQueByPPId(KeyFactorChecker.getSinglePremiumEndowmentLifeID(),
					...
					for (ProductProcessQuestionLink ppQuesLink : ppQueLinkList) {
						...
						surveyAnswer.setSurveyType(SurveyType.SINGLE_PERMIUM_ENDOWMENT_LIFE_SURVEY);
	...
	public String getPageHeader() {
		return (isEndorse ? "Life Endorsement"
				: isSinglePremiumEndowmentLife ? "Single Premium Endowment Life"
						: isFarmer ? "Farmer"
								: isPublicTermLife ? "PublicTermLife" : isPersonalAccident ? "Personal Accident" : isShortEndowLife ? "Short Term Endowment Life" : "Life")
				+ " Proposal Survey";
	}
}
