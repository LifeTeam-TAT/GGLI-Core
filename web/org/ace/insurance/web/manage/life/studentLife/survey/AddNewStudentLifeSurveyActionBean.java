...
public class AddNewStudentLifeSurveyActionBean extends BaseBean implements Serializable {
	...
	public String addNewSurvey() {
		...
		try {
			if (validateMedicalCheckUpAttach()) {
				...
				WorkflowReferenceType referenceType = WorkflowReferenceType.LIFE_PROPOSAL;
	...
	public boolean validateMedicalCheckUpAttach() {
		boolean valid = true;
		double totalSI = lifeSurvey.getLifeProposal().getTotalSumInsured();
		if ((totalSI > 10000000) && lifeSurvey.getLifeProposal().getCustomerMedicalCheckUpAttachmentList().isEmpty()) {
			valid = false;
			isMedicalCheckUpAtt = true;
			addErrorMessage("surveyEntryForm:medicalChkUpImageGrid", MessageId.MEDICAL_CHECK_UP_IS_REQUIRED);
		} else if ((totalSI >= 5000000 && totalSI <= 10000000) && lifeSurvey.getLifeProposal().getCustomerMedicalCheckUpAttachmentList().isEmpty()) {
			if (lifeSurvey.getLifeProposal().getCustomer().getAgeForNextYear() > 35) {
				valid = false;
				isMedicalCheckUpAtt = true;
				addErrorMessage("surveyEntryForm:medicalChkUpImageGrid", MessageId.MEDICAL_CHECK_UP_IS_REQUIRED);
			}
	...
	public void setProposalInsuredPerson(ProposalInsuredPerson insuredPerson) {
		ProductProcessCriteriaDTO productProcessCriteriaDTO = new ProductProcessCriteriaDTO();
		...
		if (insuredPerson == null) {
			if (lifeSurvey.getLifeProposal().getCustomerSurveyQuestionAnswerList() != null && !lifeSurvey.getLifeProposal().getCustomerSurveyQuestionAnswerList().isEmpty()) {
				...
				productProcessCriteriaDTO.setStudentAgeType(StudentAgeType.PARENT_SQ);
				List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService.findPProQueByPPId(ProductIDConfig.getStudentLifeId(),
						ProductProcessIDConfig.getProposalProcessId(), productProcessCriteriaDTO);
			...
			if (proposalInsuredPerson.getSurveyQuestionAnswerList() != null && !proposalInsuredPerson.getSurveyQuestionAnswerList().isEmpty()) {
				...
				if (insuredPerson.getAgeForNextYear() > 3) {
					productProcessCriteriaDTO.setStudentAgeType(StudentAgeType.ABOVE_3_YEARS);
				} else {
					productProcessCriteriaDTO.setStudentAgeType(StudentAgeType.UNDER_3_YEARS);
				}
				List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService.findPProQueByPPId(ProductIDConfig.getStudentLifeId(),
						ProductProcessIDConfig.getProposalProcessId(), productProcessCriteriaDTO);
				for (ProductProcessQuestionLink ppQuesLink : ppQueLinkList) {
