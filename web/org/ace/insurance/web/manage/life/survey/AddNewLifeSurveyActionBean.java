...
public class AddNewLifeSurveyActionBean extends BaseBean implements Serializable {
	...
	public void init() {
		...
		isPersonalAccident = (KeyFactorChecker.isPersonalAccident(product) || KeyFactorChecker.isPersonalAccidentUSD(product)) ? true : false;
	...
	public void generateReport() {
		...
		if (isPersonalAccident) {
			...
			if (proposal.getProposalInsuredPersonList().size() > 1) {
				DocumentBuilder.generatePersonalAccidentGroupSanction(proposal, dirPath, fileName);
			} else {
				DocumentBuilder.generatePersonalAccidentSanction(proposal, dirPath, fileName);
			}
		} else if (isFarmer) {
			fileName = "Farmer_" + customerName + "_Sanction" + ".pdf";
			DocumentBuilder.generateFarmerSanction(proposal, dirPath, fileName);
			...
			ShortEndowUnderwritingDOCFactory.generateShortEndowLifeSanction(proposal, dirPath, fileName);
		} else if (isSimpleLife) {
			fileName = "SimpleLife_" + customerName + "_Sanction" + ".pdf";
			DocumentBuilder.generateSimpleLifeSanction(proposal, dirPath, fileName);
		} else {
			if (isSinglePrint()) {
				fileName = (isPublicLife ? "PublicLife_" : isSnakeBite ? "SnakeBite_" : "SportMan_") + customerName + "_Sanction" + ".pdf";
				DocumentBuilder.generatePublicLifeSanction(proposal, dirPath, fileName);
			} else {
				fileName = "GroupLife_" + customerName + "_Sanction" + ".pdf";
				DocumentBuilder.generateGroupLIfeSanction(proposal, user.getName(), dirPath, fileName);
			}
		}
	...
	private boolean isSinglePrint() {
		if (lifeSurvey.getLifeProposal().getProposalInsuredPersonList().size() == 1) {
			return true;
		}
		return false;
	}
	...
	public void selectUser() {
		...
		WorkFlowType workFlowType = isPersonalAccident ? WorkFlowType.PERSONAL_ACCIDENT
				: isFarmer ? WorkFlowType.FARMER : isSimpleLife ? WorkFlowType.SIMPLE_LIFE : isShortEndowLife ? WorkFlowType.SHORT_ENDOWMENT : WorkFlowType.LIFE;
	...
	public void setProposalInsuredPerson(ProposalInsuredPerson insuredPerson) {
		...
		if (proposalInsuredPerson.getSurveyQuestionAnswerList() != null && !proposalInsuredPerson.getSurveyQuestionAnswerList().isEmpty()) {
			for (SurveyQuestionAnswer sqanswer : proposalInsuredPerson.getSurveyQuestionAnswerList()) {
				...
				surveyAnswerDTO.setSurveyType(SurveyType.SHORT_ENDOWMENT_LIFE_SURVEY);
				surveyQuestionAnswerDTOList.add(surveyAnswerDTO);
			}
		} else {
			List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService.findPProQueByPPId(ProductIDConfig.getShortEndowLifeId(),
	...
	public String getPageHeader() {
		return (isEndorse ? "Life Endorsement"
				: isFarmer ? "Farmer" : isPublicTermLife ? "PublicTermLife" : isPersonalAccident ? "Personal Accident" :isSimpleLife ? "Simple Life" : isShortEndowLife ? "Short Term Endowment Life" : "Life")
				+ " Proposal Survey";
	}
}
