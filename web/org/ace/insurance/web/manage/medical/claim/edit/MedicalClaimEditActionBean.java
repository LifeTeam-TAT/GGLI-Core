...
public class MedicalClaimEditActionBean extends BaseBean implements Serializable {
	...
	private boolean deathGuardian;
	private boolean insuredPerson;
	private boolean beneficiary;
	private boolean guardian;
	private boolean successor;
	private boolean existBeneficiary;
	private String remark;
	private User responsiblePerson;
	private WorkflowTask workFlowTask;
	private List<String> selectedClaimTypes;
	...
	private MedicalPolicyInsuredPersonBeneficiaryDTO selectedBeneficiaryDTO;
	private MedicalClaimBeneficiaryDTO medicalClaimBeneficiaryDTO;
	private List<MedicalPolicyInsuredPersonBeneficiaryDTO> policyInsuredPersonBeneficiaryList;
	private List<WorkFlowHistory> workFlowHistoryList;
	private List<String> items;
	private Map<String, MedicalClaimDTO> map = new HashMap<String, MedicalClaimDTO>();
	...
	private Validator<MedicalPolicyInsuredPersonBeneficiaryDTO> deathBeneInfoValidator;

	public void setDeathBeneInfoValidator(Validator<MedicalPolicyInsuredPersonBeneficiaryDTO> deathBeneInfoValidator) {
	...
	public void init() {
		...
		selectedClaimTypes = new ArrayList<String>();
		...
		if (medicalClaimProposalDTO.isHospitalized()) {
			...
			selectedClaimTypes.add("Hospitalization");
		}
		if (medicalClaimProposalDTO.isOperation()) {
			items.add("Operation");
			selectedClaimTypes.add("Operation");
		}
		if (medicalClaimProposalDTO.isMedication()) {
			items.add("Medication");
			selectedClaimTypes.add("Medication");
		}
		if (medicalClaimProposalDTO.isDeath()) {
			items.add("Death");
			selectedClaimTypes.add("Death");
		}
		for (MedicalClaimBeneficiaryDTO mcb : medicalClaimProposalDTO.getMedicalClaimBeneficiariesList()) {
			if (mcb.getName() != null) {
				medicalClaimBeneficiaryDTO.setBeneficiaryRole(MedicalBeneficiaryRole.SUCCESSOR);
				medicalClaimBeneficiaryDTO.setName(mcb.getName());
				medicalClaimBeneficiaryDTO.setNrc(mcb.getNrc());
				System.out.println("Pol" + mcb.getRelationShip());
				medicalClaimBeneficiaryDTO.setRelationShip(mcb.getRelationShip());
				medicalClaimBeneficiaryDTO.setPercentage(mcb.getPercentage());
			}
			if (mcb.getMedPolicyInsuBeneDTO() != null) {
				mcb.getMedPolicyInsuBeneDTO().setClaimBeneficiary(true);
			}
			if (mcb.getMedPolicyInsuBeneDTO() != null) {
				existBeneficiary = true;
			}
		}
	}
	...
	private void initializeInjection() {
		...
		medicalClaimProposalDTO = (medicalClaimProposalDTO == null) ? (MedicalClaimProposalDTO) getParam("medicalClaimProposalDTO") : medicalClaimProposalDTO;
		WorkFlow workFlow = workFlowService.findWorkFlowByRefNo(medicalClaimProposalDTO.getId());
		policyInsuredPersonBeneficiaryList = new ArrayList<MedicalPolicyInsuredPersonBeneficiaryDTO>();
		for (MedicalPolicyInsuredPersonBeneficiaryDTO mpb : medicalClaimProposalDTO.getPolicyInsuredPersonDTO().getPolicyInsuredPersonBeneficiariesDtoList()) {
			if (mpb.isDeath()) {
		...
		if (workFlow != null) {
			responsiblePerson = workFlow.getResponsiblePerson();
			workFlowTask = workFlow.getWorkflowTask();

		}
		if (medicalClaimProposalDTO.getDeathClaimDTO() != null) {
			map.put("Death", new MedicalClaimDTO(medicalClaimProposalDTO.getDeathClaimDTO()));
		}
		if (medicalClaimProposalDTO.getOperationClaimDTO() != null) {
			map.put("Operation", new OperationClaimDTO(medicalClaimProposalDTO.getOperationClaimDTO()));
		}
		if (medicalClaimProposalDTO.getDeathClaimDTO() != null) {
			map.put("Medication", new DeathClaimDTO(medicalClaimProposalDTO.getMedicationClaimDTO()));
		}
		if (medicalClaimProposalDTO.getHospitalizedClaimDTO() != null) {
			map.put("Hospitalization", new HospitalizedClaimDTO(medicalClaimProposalDTO.getHospitalizedClaimDTO()));
		}
	}

	public boolean getSaleMan() {
		if (medicalClaimProposalDTO.getAgent() != null) {
			saleMan = false;
		} else if (medicalClaimProposalDTO.getSaleman() != null) {
			saleMan = true;
		}
		return saleMan;
	}
	...
	public void changeGuardion(AjaxBehaviorEvent event) {
		if (deathGuardian) {
			beneficiary = true;
		} else {
			beneficiary = false;
	...
	public String onFlowProcess(FlowEvent event) {
		...
		boolean valid = true;
		if ("generalInfo".equals(event.getOldStep()) && "beneficiaryClaimInfo".equals(event.getNewStep())) {
			if (medicalClaimProposalDTO.isHospitalized()) {
				if (medicalClaimProposalDTO.getHospitalizedClaimDTO().getHospitalizedStartDate()
						.after(medicalClaimProposalDTO.getHospitalizedClaimDTO().getHospitalizedEndDate())) {
					addErrorMessage(formID + ":startDate", MessageId.HOSPITALIZED_DATE_INFO);
				}
			}
			if (medicalClaimProposalDTO.isOperation() && medicalClaimProposalDTO.isDeath()) {
				if (medicalClaimProposalDTO.getOperationClaimDTO().getOperationDate().after(medicalClaimProposalDTO.getDeathClaimDTO().getDeathDate())) {
					addErrorMessage(formID + ":operationDate", MessageId.OPERATION_DATE_INFO);
				}
			}
			if (medicalClaimProposalDTO.isHospitalized() && medicalClaimProposalDTO.isDeath()) {
				if (medicalClaimProposalDTO.getHospitalizedClaimDTO().getHospitalizedStartDate().after(medicalClaimProposalDTO.getDeathClaimDTO().getDeathDate())) {
					addErrorMessage(formID + ":startDate", MessageId.HOSPITALIZED_STARTDATE_INFO);
				}
				if (medicalClaimProposalDTO.getHospitalizedClaimDTO().getHospitalizedEndDate().after(medicalClaimProposalDTO.getDeathClaimDTO().getDeathDate())) {
					addErrorMessage(formID + ":endDate", MessageId.HOSPITALIZED_ENDDATE_INFO);
				}
			}
			if (medicalClaimProposalDTO.isMedication() && medicalClaimProposalDTO.isDeath()) {
				if (medicalClaimProposalDTO.getMedicationClaimDTO().getReceivedDate().after(medicalClaimProposalDTO.getDeathClaimDTO().getDeathDate())) {
	...
	public String submitClaimInfo() {
		...
		try {
			...
			medicalClaimProposalDTO = medicalClaimRequestFrontService.updatMedicalClaim(medicalClaimProposalDTO, workFlowDTO);
	...
	public void returnSaleMan(SelectEvent event) {
		...
		medicalClaimProposalDTO.setSaleman(saleMan);
	}

	public void returnAgent(SelectEvent event) {
		Agent agent = (Agent) event.getObject();
		medicalClaimProposalDTO.setAgent(agent);
	}

	public void returnBranch(SelectEvent event) {
		Branch branch = (Branch) event.getObject();
		medicalClaimProposalDTO.setBranch(branch);
	}

	// public void returnHospitalizedReasonDialog(SelectEvent event) {
	// ICD10 icd10 = (ICD10) event.getObject();
	// medicalClaimProposalDTO.getHospitalizedClaimDTO().setHospitalizedReason(icd10);
	// }

	public void returnMedicalPlaceDialog(SelectEvent event) {
		Hospital medicalPlace = (Hospital) event.getObject();
		medicalClaimProposalDTO.getHospitalizedClaimDTO().setMedicalPlace(medicalPlace);
	}

	public void returnOperationDialog(SelectEvent event) {
		Operation operation = (Operation) event.getObject();
		medicalClaimProposalDTO.getOperationClaimDTO().setOperation(operation);
	}

	public void returnOperationReasonDialog(SelectEvent event) {
		ICD10 icd10 = (ICD10) event.getObject();
		medicalClaimProposalDTO.getOperationClaimDTO().setOperationReason(icd10);
	}

	public void returnMedicationReasonDialog(SelectEvent event) {
		ICD10 icd10 = (ICD10) event.getObject();
		medicalClaimProposalDTO.getMedicationClaimDTO().setMedicationReason(icd10);
	}

	// public void returnDeathReasonDialog(SelectEvent event) {
	// ICD10 icd10 = (ICD10) event.getObject();
	...
	public void setMedicalClaimBeneficiaryDTO(MedicalClaimBeneficiaryDTO medicalClaimBeneficiaryDTO) {
		this.medicalClaimBeneficiaryDTO = medicalClaimBeneficiaryDTO;
	}
	...
	public void selectedDeathPlace() {
		if (medicalClaimProposalDTO.getDeathClaimDTO().getDeathPlace().equalsIgnoreCase("Other")) {
			other = true;
		} else
			other = false;
	}
	...
	public boolean isGuardian() {
		for (MedicalClaimBeneficiaryDTO mcb : medicalClaimProposalDTO.getMedicalClaimBeneficiariesList())
			if (mcb.getMedPolicyGuardianDTO() != null) {
				guardian = true;
			}
		return guardian;
	}
	...
	public boolean isSuccessor() {
		for (MedicalClaimBeneficiaryDTO mcb : medicalClaimProposalDTO.getMedicalClaimBeneficiariesList()) {
			if (mcb.getBeneficiaryRole().equals(MedicalBeneficiaryRole.SUCCESSOR)) {
				successor = true;
			}
		}
		return successor;
	...
	public boolean isInsuredPerson() {
		for (MedicalClaimBeneficiaryDTO mcb : medicalClaimProposalDTO.getMedicalClaimBeneficiariesList())
			if (mcb.getMedicalPolicyInsuredPersonDTO() != null) {
				insuredPerson = true;
			}
		return insuredPerson;
	}

	public void setInsuredPerson(boolean insuredPerson) {
	...
	public void prepareEditDeathBeneficiary(MedicalPolicyInsuredPersonBeneficiaryDTO medPolInsPerBene) {
		this.selectedBeneficiaryDTO = medPolInsPerBene;
	}

	public void changeClaimType() {
		if (selectedClaimTypes != null) {
			if (!selectedClaimTypes.contains("Hospitalization")) {
				medicalClaimProposalDTO.setHospitalizedClaimDTO(null);
				medicalClaimProposalDTO.setHospitalized(false);
			} else {
				if (map.get("Hospitalization") != null) {
					medicalClaimProposalDTO.setHospitalizedClaimDTO((HospitalizedClaimDTO) map.get("Hospitalization"));
				} else {
					medicalClaimProposalDTO.setHospitalizedClaimDTO(new HospitalizedClaimDTO());
				}
			...
			if (!selectedClaimTypes.contains("Operation")) {
				...
				if (map.get("Operation") != null) {
					medicalClaimProposalDTO.setOperationClaimDTO((OperationClaimDTO) map.get("Operation"));
				} else {
					medicalClaimProposalDTO.setOperationClaimDTO(new OperationClaimDTO());
				}
				medicalClaimProposalDTO.setOperation(true);
			}
			if (!selectedClaimTypes.contains("Medication")) {
				medicalClaimProposalDTO.setMedicationClaimDTO(null);
				medicalClaimProposalDTO.setMedication(false);
			} else {
				if (map.get("Medication") != null) {
					medicalClaimProposalDTO.setMedicationClaimDTO((MedicationClaimDTO) map.get("Medication"));
				} else {
			...
			if (!selectedClaimTypes.contains("Death")) {
				...
				medicalClaimProposalDTO.setDeath(false);
			} else {
				if (map.get("Death") != null) {
					medicalClaimProposalDTO.setDeathClaimDTO((DeathClaimDTO) map.get("Death"));
				} else {
					medicalClaimProposalDTO.setDeathClaimDTO(new DeathClaimDTO());
				}
				medicalClaimProposalDTO.setDeath(true);
			}
		}
	}

	public void updateDeathBeneficiary() {
		if (selectedBeneficiaryDTO.isDeath()) {
			ValidationResult result = deathBeneInfoValidator.validate(selectedBeneficiaryDTO);
	...
	public boolean isChild() {
		if (medicalClaimProposalDTO.getPolicyInsuredPersonDTO().getGuardian() != null) {
			child = true;
		}
		return child;
	}
	...
	public boolean isDeathGuardian() {
		if (medicalClaimProposalDTO.getPolicyInsuredPersonDTO().getGuardian() != null
				&& medicalClaimProposalDTO.getMedicalClaimBeneficiariesList().get(0).getMedPolicyInsuBeneDTO() != null) {
			deathGuardian = true;
		}
		return deathGuardian;
	}

	public void setDeathGuardian(boolean deathGuardian) {
		this.deathGuardian = deathGuardian;
	}

	public List<MedicalPolicyInsuredPersonBeneficiaryDTO> getPolicyInsuredPersonBeneficiaryList() {
		return policyInsuredPersonBeneficiaryList;
	}

	public void setPolicyInsuredPersonBeneficiaryList(List<MedicalPolicyInsuredPersonBeneficiaryDTO> policyInsuredPersonBeneficiaryList) {
		this.policyInsuredPersonBeneficiaryList = policyInsuredPersonBeneficiaryList;
	}

	public MedicalPolicyInsuredPersonBeneficiaryDTO getSelectedBeneficiaryDTO() {
		return selectedBeneficiaryDTO;
	}

	public void setSelectedBeneficiaryDTO(MedicalPolicyInsuredPersonBeneficiaryDTO selectedBeneficiaryDTO) {
		this.selectedBeneficiaryDTO = selectedBeneficiaryDTO;
	}

	public boolean isExistBeneficiary() {
