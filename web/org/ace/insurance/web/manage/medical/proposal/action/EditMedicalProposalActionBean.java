 ...
 ***************************************************************************************/
@ViewScoped
@ManagedBean(name = "EditMedicalProposalActionBean")
public class EditMedicalProposalActionBean extends BaseBean {
	...
	private int unit;
	private boolean disableSelfRS;
	private boolean isItselfCustomer;
	...
	public void init() {
		...
		for (MedProInsuDTO dto : medicalProposalDTO.getMedProInsuDTOList()) {
			existingGuardian = dto.getGuardianDTO() == null ? false : true;
		}
	}
	...
	private void loadData() {
		...
		if (medicalProposalDTO.getCustomer().getStateCode() != null) {
			townshipCodeList = townshipCodeService.findByStateCode(medicalProposalDTO.getCustomer().getStateCode());
		}
	}
	...
	public String onFlowProcess(FlowEvent event) {
		...
		if ("customerTab".equals(event.getOldStep()) && "InsuredPersonInfo".equals(event.getNewStep())) {
			...
			if (medicalProposalDTO.getCustomer().getId() == null) {
			...
			isItselfCustomer = true;
		}

		if ("InsuredPersonInfo".equals(event.getOldStep()) && "workflow".equals(event.getNewStep())) {
			ValidationResult result = insuredpersonValidator.validate(medicalProposalDTO.getMedProInsuDTOList().get(0));
			if (beneficiariesInfoDTOMap != null && beneficiariesInfoDTOMap.size() != 0) {
				...
				if (totalPercent > 100) {
					result.addErrorMessage(formID + ":beneficiariesInfoTablePanel", MessageId.OVER_BENEFICIARY_PERCENTAGE);
				}
				if (totalPercent < 100) {
					result.addErrorMessage(formID + ":beneficiariesInfoTablePanel", MessageId.LOWER_BENEFICIARY_PERCENTAGE);
				}

			} else {
				result.addErrorMessage(formID + ":beneficiariesInfoTablePanel", MessageId.REQUIRED_BENEFICIARY_PERSON);
			}
			for (ErrorMessage message : result.getErrorMeesages()) {
				addErrorMessage(message);
				valid = false;
	...
	public String updateMedicalProposal() {
		String result = null;
		try {
			WorkflowTask workflowTask = surveyAgain ? WorkflowTask.SURVEY : WorkflowTask.APPROVAL;
			WorkFlowDTO workFlowDTO = new WorkFlowDTO(medicalProposalDTO.getId(), remark, workflowTask, WorkflowReferenceType.MEDICAL_PROPOSAL, user, responsiblePerson);
	...
	private boolean fromInsuredPersonSelected;

	public void setSelfRelation(boolean isSelfRelation) {
		this.isSelfRelation = isSelfRelation;
	}

	public boolean isFromInsuredPersonSelected() {
		return fromInsuredPersonSelected;
	}

	public void setFromInsuredPersonSelected(boolean fromInsuredPersonSelected) {
		this.fromInsuredPersonSelected = fromInsuredPersonSelected;
	}
	...
	public void changeCusToInsuredPerson() {
		...
		RelationShip tempRelationShip = dto.getRelationship();
		if ("SELF".equalsIgnoreCase(dto.getRelationship().getName())) {
			CustomerDTO customer = medicalProposalDTO.getCustomer();
			fromInsuredPersonSelected = true;
			dto.setSameInsuredPerson(true);

			dto.setCustomer(customer);
			isNrcCustomer = true;
			/*
			 * if (customer.getId() != null) { insuredPersonFullName =
			 * customer.getFullName(); }
			 */
			createInsuredPersonAddOn();
		} else {
			dto = new MedProInsuDTO();
			dto.setRelationship(tempRelationShip);
			/*
			 * insuredPersonDTO.getCustomer().setMaritalStatus(MaritalStatus.
			 * SINGLE ); insuredPersonFullName = "";
			 */
			fromInsuredPersonSelected = false;
		}
	...
	public void changeCustomerGuardian() {
		if (existingGuardian) {
			CustomerDTO customer = medicalProposalDTO.getCustomer();
			MedProGuardianDTO guardianDTO = new MedProGuardianDTO(null, customer, dto.getRelationship());
			dto.setGuardianDTO(guardianDTO);
			dto.getGuardianDTO().getCustomer().setExistsEntity(true);
		} else {
	...
	public void changeSelectedCusToInsuredPerson(CustomerDTO customer) {
		dto.setCustomer(customer);
		dto.getCustomer().setExistsEntity(true);
		createInsuredPersonAddOn();
	}
	...
	private ValidationResult validBeneficiary(MedProInsuBeneDTO beneficiariesInfo) {
		...
		// setValidationFailed(!result.isVerified());
...
//	}
	...
	public void returnInsuredPersonCustomer(SelectEvent event) {
		...
		if (customer.getId().equals(medicalProposalDTO.getCustomer().getId())) {
			...
			isItselfCustomer = false;
			isSelfRelation = false;
		}
		dto.setSameInsuredPerson(false);
	...
	public void returnInsuredPersonTownShip(SelectEvent event) {
		...
		dto.getCustomer().getResidentAddress().setTownship(townShip);
	}
	...
	public void returnOccupation(SelectEvent event) {
		...
		dto.getCustomer().setOccupation(occupation);
	}
	...
	public void returnNationalityForInsuredPerson(SelectEvent event) {
		Country nationality = (Country) event.getObject();
		dto.getCustomer().setCountry(nationality);
	}
	...
	public void setDto(MedProInsuDTO dto) {
		this.dto = dto;
		addOnfig();
	}
	...
	private void setKeyFactorValue(PaymentType paymentType, int age, Gender gender) {
		for (MedicalProposalInsuredPersonKeyFactorValue vehKF : dto.getKeyFactorValueList()) {
	...
	public void saveMedicalInsuredPerson() {
		...
		ValidationResult result = insuredpersonValidator.validate(dto);
		if (result.isVerified()) {
			...
			cal.add(Calendar.YEAR, dto.getPeriodYear());
			setKeyFactorValue(medicalProposalDTO.getPaymentType(), Utils.getAgeForNextYear(dto.getCustomer().getDateOfBirth()), dto.getCustomer().getGender());
			dto.setAge(Utils.getAgeForNextYear(dto.getCustomer().getDateOfBirth()));
			dto.setEndDate(cal.getTime());
			dto.setUnit(selectedUnit.getValue());
			dto.setInsuredPersonBeneficiariesList(getProposalInsuredPersonBeneficiariesList());
			insuredPersonDTOMap.put(dto.getTempId(), dto);
	...
	public void applyProduct() {
		dto.setInsuredPersonAddOnList(selectedAddOnLists);
		insuredPersonDTOMap.put(dto.getTempId(), dto);
		createNewMedicalInsuredPerson();
	}
	...
	public void addUnit() {
		if (unit > dto.getUnit()) {
			addErrorMessage("addOnUnitDialogForm:value", MessageId.INVALID_ADDON_UNIT, dto.getUnit());
		} else {
	...
	public void addOnfig() {
		...
		if (!dto.getInsuredPersonAddOnList().isEmpty()) {
			selectedAddOnLists.addAll(dto.getInsuredPersonAddOnDTOList());
		}
		addOnList = new ArrayList<MedProInsuAddOnDTO>();
		MedProInsuAddOnDTO addOnDto = null;
		for (AddOn addOn : dto.getProduct().getAddOnList()) {
	...
	public void prepareEditInsuredPersonInfo(MedProInsuDTO insuDto) {
		this.dto = insuDto;
		this.product = insuDto.getProduct();
		if (insuDto.getInsuredPersonBeneficiariesList() != null) {
			createNewBeneficiariesInfoDTOMap();
			for (MedProInsuBeneDTO bdto : insuDto.getInsuredPersonBeneficiariesList()) {
	...
	public void addHistoryRecord() {
		dto.setHistoryRecordList(historyRecordList);
		insuredPersonDTOMap.put(dto.getTempId(), dto);
		createNewMedicalInsuredPerson();
	}
	...
	public void prepareHistoryRecord(MedProInsuDTO insuredPersonDTO) {
		...
		if (insuredPersonDTO.getHistoryRecordList() != null) {
			historyRecordList.addAll(insuredPersonDTO.getHistoryRecordList());
		}
	}
	...
	public boolean isDisableSelfRS() {
		return disableSelfRS;
	}

	public boolean isItselfCustomer() {
		return isItselfCustomer;
	}
}
