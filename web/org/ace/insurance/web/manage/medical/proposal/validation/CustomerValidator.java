...
public class CustomerValidator implements Validator<CustomerDTO> {
	...
	public ValidationResult validate(CustomerDTO customerDTO) {
		...
		if (customerDTO != null) {
			...
			if (customerDTO.getHeight() < 1) {
				result.addErrorMessage(formID + ":customerHeight", UIInput.REQUIRED_MESSAGE_ID);
			}
			if (customerDTO.getWeight() < 1) {
				result.addErrorMessage(formID + ":customerWeight", UIInput.REQUIRED_MESSAGE_ID);
			}
			...
			if (customerDTO.getPlaceOfBirth().equals("")) {
				result.addErrorMessage(formID + ":customerPlaceOfBirth", UIInput.REQUIRED_MESSAGE_ID);
			}
			if (customerDTO.getCountry() == null || ValidationUtil.isStringEmpty(customerDTO.getCountry().getName())) {
				result.addErrorMessage(formID + ":customerNationality", UIInput.REQUIRED_MESSAGE_ID);
			}
			if (customerDTO.getOccupation() == null || ValidationUtil.isStringEmpty(customerDTO.getOccupation().getName())) {
				result.addErrorMessage(formID + ":occupationcustomer", UIInput.REQUIRED_MESSAGE_ID);
			}
