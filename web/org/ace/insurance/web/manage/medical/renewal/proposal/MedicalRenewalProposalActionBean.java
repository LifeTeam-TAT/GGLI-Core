...
public class MedicalRenewalProposalActionBean extends BaseBean {
	...
	@ManagedProperty(value = "#{ProposalValidator}")
	private Validator<MedProDTO> proposalValidator;

	public void setProposalValidator(Validator<MedProDTO> proposalValidator) {
	...
	private IMedicalProposalService proposalService;
	...
	private IAddNewMedicalProposalFrontService proposalFrontService;

	public void setProposalFrontService(IAddNewMedicalProposalFrontService proposalFrontService) {
		this.proposalFrontService = proposalFrontService;
	}
	...
	private boolean existingGuardian;
	...
	private MedProInsuAddOnDTO insuredPersonAddOnDTO;
	...
	private List<MedProInsuAddOnDTO> selectedAddOnLists;
	private List<MedProInsuAddOnDTO> addOnList;
	...
	private Map<String, MedProInsuAddOnDTO> insuredPersonAddOnDTOMap;
	...
	private void initializeInjection() {
		...
		if (isExistParam("editMedicalRenewalProposal")) {
			medicalProposal = (MedicalProposal) getParam("editMedicalRenewalProposal");
			isConfirmEdit = true;
		} else if (isExistParam("editEnquiryMedicalRenewalProposal")) {
			medicalProposal = (MedicalProposal) getParam("editEnquiryMedicalRenewalProposal");
			isEnquiryEdit = true;
		} else {
			MedicalPolicy policy = (MedicalPolicy) getParam("medicalPolicy");
			medicalProposal = new MedicalProposal(policy);
			medicalProposal.setOldMedicalPolicy(policy);
			medicalProposal.setSubmittedDate(policy.getPolicyInsuredPersonList().get(0).getEndDate());
			checkProduct();
		}
	...
	public void init() {
		...
		insuredPersonAddOnDTOMap = new LinkedHashMap<String, MedProInsuAddOnDTO>();
		prepareInsuredPersonAddOn();
		...
		unitsMap = new HashMap<Integer, Unit>();
		...
		for (MedProInsuDTO insuDto : insuredPersonDTOList) {
			// insuDto.setInsuredPersonAddOnList(new
			// ArrayList<MedProInsuAddOnDTO>());
			List<MedProInsuAddOnDTO> removeList = new ArrayList<MedProInsuAddOnDTO>();
			for (MedProInsuAddOnDTO insuAddOn : insuDto.getInsuredPersonAddOnList()) {
				if (!insuDto.getProduct().getAddOnList().contains(insuAddOn.getAddOn())) {
					removeList.add(insuAddOn);
				}
			}
			for (MedProInsuAddOnDTO insAddOn : removeList) {
				if (insuDto.getInsuredPersonAddOnList().contains(insAddOn)) {
					insuDto.getInsuredPersonAddOnList().remove(insAddOn);
				}
			}
			if (medicalProposal.getOldMedicalPolicy() != null) {
				if (insuDto.getCustomer().getId().equals(medicalProposal.getOldMedicalPolicy().getCustomer().getId())) {
					insuDto.setSameInsuredPerson(true);
				}
			}
			insuredPersonDTOMap.put(insuDto.getTempId(), insuDto);
			insuredPersonDTO.setPeriodYear(insuDto.getPeriodYear());
			selectedUnit = new Unit();
			selectedUnit.setValue(insuDto.getUnit());
			selectedUnit.setKey(insuDto.getUnit() + "");
		}

		for (MedProInsuDTO insuredPersonDTO : medicalProposalDTO.getMedProInsuDTOList()) {
			existingGuardian = insuredPersonDTO.getGuardianDTO() == null ? false : true;
		}
	}
	...
	public String onFlowProcess(FlowEvent event) {
		...
		if ("InsuredPersonInfo".equals(event.getOldStep()) && "workFlow".equals(event.getNewStep())) {
			if (insuredPersonDTOMap.size() < 1) {
				if (CustomerType.INDIVIDUALCUSTOMER.equals(medicalProposalDTO.getCustomerType())) {
					addErrorMessage(formID + ":insuredPersonInfoTable", MessageId.REQUIRE_INSUREDPERSON);
				} else {
					addErrorMessage(formID + ":insuredPersonInfoTable", MessageId.REQUIRE_TWO_INSUREDPERSON);
				}
				valid = false;
			}
		}
	...
	public boolean isExistingGuardian() {
	...
	public MedProInsuAddOnDTO getInsuredPersonAddOnDTO() {
		return insuredPersonAddOnDTO;
	}

	public void setInsuredPersonAddOnDTO(MedProInsuAddOnDTO insuredPersonAddOnDTO) {
	...
	public void changeIdTypeBene(AjaxBehaviorEvent e) {
		IdType idType = (IdType) ((UIOutput) e.getSource()).getValue();
	...
	public String submitRenewalProposal() {
		String result = null;
		try {
			WorkflowTask workflowTask = isConfirmEdit ? surveyAgain ? WorkflowTask.RENEWAL_SURVEY : WorkflowTask.RENEWAL_APPROVAL : WorkflowTask.RENEWAL_SURVEY;
			medicalProposalDTO.setProposalType(ProposalType.RENEWAL);
			WorkFlowDTO workFlowDTO = new WorkFlowDTO(medicalProposalDTO.getId(), remark, workflowTask, WorkflowReferenceType.MEDICAL_RENEWAL_PROPOSAL, user, responsiblePerson);
			medicalProposalDTO.setMedProInsuDTOList(getInsuredPersonDtoList());
			ExternalContext extContext = getFacesContext().getExternalContext();
			if (isConfirmEdit) {
				medicalProposal = proposalFrontService.updateMedicalProposal(medicalProposalDTO, workFlowDTO, bancaassuranceProposal);
				extContext.getSessionMap().put(Constants.MESSAGE_ID, MessageId.EDIT_POLICY_RENEWAL_PROCESS_SUCCESS);
			} else if (isEnquiryEdit) {
				medicalProposal = proposalFrontService.updateMedicalProposal(medicalProposalDTO, null, bancaassuranceProposal);
				extContext.getSessionMap().put(Constants.MESSAGE_ID, MessageId.EDIT_POLICY_RENEWAL_PROCESS_SUCCESS);
			} else {
				medicalProposal = proposalFrontService.addNewMedicalProposal(medicalProposalDTO, workFlowDTO, RequestStatus.PROPOSED.name(), bancaassuranceProposal);
				extContext.getSessionMap().put(Constants.MESSAGE_ID, MessageId.RENEWAL_PROCESS_SUCCESS);
			}
	...
	public void changeCusToInsuredPerson() {
		...
		if (insuredPersonDTO.getRelationship() != null) {
			...
			if (getSelfRelationship().equals(insuredPersonDTO.getRelationship())) {
				if (insuredPersonDTOMap.size() != 0) {
					for (MedProInsuDTO dto : getInsuredPersonDtoList()) {
	...
	public void changeCustomerGuardian() {
		if (existingGuardian) {
			CustomerDTO customer = medicalProposalDTO.getCustomer();
			MedProGuardianDTO guardianDTO = new MedProGuardianDTO(null, customer, insuredPersonDTO.getRelationship());
	 ...
	 **************************/
	public void prepareInsuredPersonAddOn() {
		createInsuredPersonAddOn();
	}

	public List<MedProInsuAddOnDTO> getInsuredPersonAddOnList() {
		return new ArrayList<MedProInsuAddOnDTO>(insuredPersonAddOnDTOMap.values());
	}

	private void createInsuredPersonAddOn() {
		insuredPersonAddOnDTO = new MedProInsuAddOnDTO();
	}

	public void saveInsuredPersonAddOn() {
		if (validInsuredPersonAddOn(insuredPersonAddOnDTO)) {
			insuredPersonAddOnDTOMap.put(insuredPersonAddOnDTO.getAddOn().getId(), insuredPersonAddOnDTO);
			createInsuredPersonAddOn();
	...
	private boolean validInsuredPersonAddOn(MedProInsuAddOnDTO insuredPersonAddOn) {
		boolean valid = true;
		if (insuredPersonAddOn.getAddOn() == null) {
	...
	public void checkInsuredPerson() {
		for (MedProInsuDTO insuDTO : getInsuredPersonDtoList()) {
	...
	public void resetResponsiblePerson() {
		responsiblePerson = null;
		FacesContext.getCurrentInstance().getPartialViewContext().getRenderIds().add("medicalProposalEntryForm:responsiblePersonPanel");
	}
	...
	public List<MedProInsuAddOnDTO> getAddOnList() {
		return addOnList;
	}
	...
	// This method is only for old policy (before April 2016)
	// old policy has only one keyfactor
	public void checkProduct() {
		for (MedicalProposalInsuredPerson insuredPerson : medicalProposal.getMedicalProposalInsuredPersonList()) {
			if (insuredPerson.getKeyFactorValueList().size() < 3) {
	...
	private RelationShip getSelfRelationship() {
		...
		for (RelationShip rs : relationshipList) {
			if ("SELF".equalsIgnoreCase(rs.getName())) {
				result = rs;
			}
		}
		return result;
	...
	private boolean checkInsuredPersonByCustomerType() {
		...
		if (noOfInsuredPerson != 0) {
			...
			if (CustomerType.INDIVIDUALCUSTOMER.equals(medicalProposalDTO.getCustomerType()) && !createEdit) {
	...
	public List<MedProInsuAddOnDTO> getSelectedAddOnLists() {
		return selectedAddOnLists;
	}

	public void setSelectedAddOnLists(List<MedProInsuAddOnDTO> selectedAddOnLists) {
		this.selectedAddOnLists = selectedAddOnLists;
	}
	...
	public void onRowSelect(SelectEvent event) {
		insuredPersonAddOnDTO = (MedProInsuAddOnDTO) event.getObject();
		PrimeFaces.current().executeScript("PF('addOnUnitDialog').show();");
	...
	public void addUnit() {
		if (unit < 1) {
			addErrorMessage("addOnUnitDialogForm:value", UIInput.REQUIRED_MESSAGE_ID);
		} else if (unit > insuredPersonDTO.getUnit()) {
			addErrorMessage("addOnUnitDialogForm:value", MessageId.INVALID_ADDON_UNIT, insuredPersonDTO.getUnit());
		} else {
			insuredPersonAddOnDTO.setUnit(unit);
			unit = 0;
			PrimeFaces.current().executeScript("PF('addOnUnitDialog').hide();");
		}
	}

	public void addOnfig() {
		selectedAddOnLists = new ArrayList<MedProInsuAddOnDTO>();
		if (!insuredPersonDTO.getInsuredPersonAddOnList().isEmpty()) {
			selectedAddOnLists.addAll(insuredPersonDTO.getInsuredPersonAddOnDTOList());
		}
		addOnList = new ArrayList<MedProInsuAddOnDTO>();
		MedProInsuAddOnDTO addOnDto = null;
		for (AddOn addOn : insuredPersonDTO.getProduct().getAddOnList()) {
			addOnDto = new MedProInsuAddOnDTO(addOn);
			if (!ProductIDConfig.getMedAddOn1().equals(addOn.getId())) {
				addOnList.add(addOnDto);
			}
		}
		for (MedProInsuAddOnDTO selectedAddOn : selectedAddOnLists) {
			for (MedProInsuAddOnDTO addOn : addOnList) {
				if (addOn.getAddOn().equals(selectedAddOn.getAddOn())) {
	...
	public void prepareEditInsuredPersonInfo(MedProInsuDTO insuDto) {
		this.insuredPersonDTO = new MedProInsuDTO(insuDto);
	...
	public void handleAddOnUnitDialogClose() {
		unit = 0;
		selectedAddOnLists.remove(insuredPersonAddOnDTO);
		PrimeFaces.current().executeScript("PF('addOnUnitDialog').hide();");
	}
	...
	public String getInsuOccupation() {
		return this.insuredPersonDTO.getCustomer().getOccupation() == null ? "" : this.insuredPersonDTO.getCustomer().getOccupation().getName();
	}

	public String getInsuCountry() {
		return this.insuredPersonDTO.getCustomer().getCountry() == null ? "" : this.insuredPersonDTO.getCustomer().getCountry().getName();
	}

	public String getInsuResiTownship() {
		return this.insuredPersonDTO.getCustomer().getResidentAddress().getTownship() == null ? ""
				: this.insuredPersonDTO.getCustomer().getResidentAddress().getTownship().getName();
	}

	public String getPageHeader() {
		return (isConfirmEdit ? "Confirm Edit" : isEnquiryEdit ? "Enquiry Edit" : "Add New") + " Health Renewal Proposal";
	}
