...
public class MedicalRenewalSurveyActionBean extends BaseBean {
	...
	public void init() {
		...
		/* load surveyQuestion */
		List<ProductProcessQuestionLink> ppQueLinkList = medicalProposalService.findPProQueByPPId(ProductProcessIDConfig.getMedicalProductId(),
	...
	public String addNewSurvey() {
		...
		try {
			...
			if (result.isVerified()) {
				loadAttachment();
				loadMedicalHistory();
				WorkflowTask workflowTask = WorkflowTask.RENEWAL_APPROVAL;
				WorkFlowDTO workFlowDTO = new WorkFlowDTO(medicalProposalDTO.getId(), remark, workflowTask, WorkflowReferenceType.MEDICAL_RENEWAL_PROPOSAL, user,
						responsiblePerson);
