...
public class SurveyQuestionAnswerDTOFactroy {
	public static SurveyQuestionAnswerDTO getSurveyQuestionAnswerDTO(SurveyQuestionAnswer surveyQuestionAnswer) {
		...
		for (ResourceQuestionAnswer rs : surveyQuestionAnswer.getResourceQuestionList()) {
			...
			if (rs.getValue() == 1
					&& (InputType.SELECT_MANY_CHECKBOX.equals(surveyQuestionAnswer.getInputType()) || InputType.SELECT_MANY_MENU.equals(surveyQuestionAnswer.getInputType()))) {
				surveyQuestionAnswerDTO.getSelectedResourceQAnsDTOList().add(resourceQuestionAnswerDTO);
			} else if (rs.getValue() == 1
					&& (InputType.SELECT_ONE_MENU.equals(surveyQuestionAnswer.getInputType()) || InputType.SELECT_ONE_RADIO.equals(surveyQuestionAnswer.getInputType()))) {
				surveyQuestionAnswerDTO.setSelectedResourceQAnsDTO(resourceQuestionAnswerDTO);
			}
			surveyQuestionAnswerDTO.addResourceQuestionList(resourceQuestionAnswerDTO);
		}
		if (surveyQuestionAnswer.getCommonCreateAndUpateMarks() != null) {
			surveyQuestionAnswerDTO.setCommonCreateAndUpateMarks(surveyQuestionAnswer.getCommonCreateAndUpateMarks());
		}
	...
	public static SurveyQuestionAnswer getSurveyQuestionAnswer(SurveyQuestionAnswerDTO surveyQuestionAnswerDTO) {
		...
		if (surveyQuestionAnswerDTO.getCommonCreateAndUpateMarks() != null) {
			surveyQuestionAnswer.setCommonCreateAndUpateMarks(surveyQuestionAnswerDTO.getCommonCreateAndUpateMarks());
		}
