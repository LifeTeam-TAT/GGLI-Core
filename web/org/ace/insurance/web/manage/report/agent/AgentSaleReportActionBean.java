...
public class AgentSaleReportActionBean extends BaseBean implements Serializable {
	...
	public void reset() {
		...
		criteria.setInsuranceType(AgentSaleInsuranceType.LIFE);
	...
	public void exportExcel() {
		...
		if (null != reportList && reportList.size() > 0) {
			...
			addErrorMessage("reportList:lifeInsuranceTable", MessageId.THERE_IS_NO_DATA);
		}
	}
	...
	private class ExportExcel {
		...
		public void generate(OutputStream op) {
			try {
				...
				ExcelUtils.fillCompanyLogoAndAddress(wb, sheet, 4, 15, 18);
				...
				for (AgentSaleReport asr : reportList) {
					...
					branchCell = row.createCell(21);
					branchCell.setCellValue(asr.getBranchName());
					branchCell.setCellStyle(textCellStyle);

					salePointCell = row.createCell(22);
					salePointCell.setCellValue(asr.getSalePointName());
	...
	public void exportExcel1() {
		...
		if (null != reportList && reportList.size() > 0) {
			...
			addErrorMessage("agentSaleReportForm:lifeInsuranceTable", MessageId.THERE_IS_NO_DATA);
		}
	}
	...
	private class ExportExcel1 {
		...
		public void generate(OutputStream op) {
			try {
				...
				ExcelUtils.fillCompanyLogoAndAddress(wb, sheet, 5, 23, 26);
				row = sheet.getRow(0);
				cell = row.getCell(0);
				...
				for (AgentSaleReport asr : reportList) {
					...
					branchNameCell = row.createCell(24);
					branchNameCell.setCellValue(asr.getBranchName());
					branchNameCell.setCellStyle(textCellStyle);

					salePointNameCell = row.createCell(25);
					salePointNameCell.setCellValue(asr.getSalePointName());
