...
public class LifePremiumPaymentReportActionBean extends BaseBean implements Serializable {
	...
	public void filter() {
		...
		if (valid) {
			...
			// lazyModel = new
			// LazyDataModelUtil<LifePremiumPaymentReport>(premiumPaymentList);
		}
	}
	...
	public void exportExcel() {
		if (premiumPaymentList.size() > 0) {
			...
			addErrorMessage("premiumList:premiumTable", MessageId.THERE_IS_NO_DATA);
		}
	...
	private class ExportExcel {
		...
		public void generate(OutputStream op) {
			try {
				...
				ExcelUtils.fillCompanyLogoAndAddress(wb, sheet, 3, 6, 9);
				row = sheet.getRow(0);
				cell = row.getCell(0);
				if (criteria.getBranch() == null) {
				...
				for (List<LifePremiumPaymentReport> branchList : map.values()) {
					...
					for (LifePremiumPaymentReport LifePremiumPaymentReport : branchList) {
						...
						cell.setCellValue(LifePremiumPaymentReport.getSalePointName());
						cell.setCellStyle(textCellStyle);
						i = i + 1;
					}
	...
	public List<LifePremiumPaymentReport> getPremiumPaymentList() {
