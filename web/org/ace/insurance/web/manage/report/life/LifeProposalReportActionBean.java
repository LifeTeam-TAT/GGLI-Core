...
public class LifeProposalReportActionBean extends BaseBean implements Serializable {
	...
	public void exportExcel() {
		if (null != lifeProposalReportList && lifeProposalReportList.size() > 0) {
			...
			addErrorMessage("proposalList:lifeProposalTable", MessageId.THERE_IS_NO_DATA);
		}
	...
	private class ExportExcel {
		...
		public void generate(OutputStream op) {
			try {
				...
				ExcelUtils.fillCompanyLogoAndAddress(wb, sheet, 4, 10, 14);
				row = sheet.getRow(0);
				cell = row.getCell(0);
				if (criteria.getBranch() == null) {
				...
				for (List<LifeProposalReport> branchList : map.values()) {
					...
					for (LifeProposalReport lifeProposalReport : branchList) {
						...
						cell.setCellValue(lifeProposalReport.getSalePointName());
						cell.setCellStyle(textCellStyle);
