...
public class NewStudentLifeMonthlyReportExcel {
	private XSSFWorkbook wb;
	StudentLifeCriteria studentLifeCriteria;
	// private FormulaEvaluator evaluator ;

	public NewStudentLifeMonthlyReportExcel() {
	}

	private void load() {
		String excelTitle = studentLifeCriteria.isDaily() ? "New_StudentLife_Daily_Report" : "New_StudentLife_Monthly_Report";
		try {
	...
	public void generate(OutputStream op, List<NewStudentLifeView> newStudentLifeViewList) {
		try {
			Sheet sheet = wb.getSheet("NewStudentLife");
			...
			ExcelUtils.fillCompanyLogoAndAddress(wb, sheet, 4, 15, 20);
			row = sheet.getRow(0);
			cell = row.getCell(0);
			...
			if (studentLifeCriteria.isDaily()) {
				...
				builder.append(" \n \n New Business Daily Report (Student Life) " + start + end);
			} else {
				...
				builder.append(" \n \n New Business Monthly Report (Student Life) (" + start + end + ") - " + studentLifeCriteria.getYear());
			}
			...
			for (NewStudentLifeView report : newStudentLifeViewList) {
				count = count + 1;
				i = i + 1;
				...
				cell.setCellValue(report.getChannel());
				cell.setCellStyle(defaultCellStyle);
