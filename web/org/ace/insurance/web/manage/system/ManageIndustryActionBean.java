...
public class ManageIndustryActionBean extends BaseBean implements Serializable {
	...
	public void search() {
		industrieList = industryService.findByCriteria(criteria);
	}
	...
	public void createNewIndustry() {
		...
		industry = new Industry();
		criteria = "";
		PrimeFaces.current().resetInputs(":industryEntryForm");
	}
	...
	public boolean isAlreadyExistIndustry() {
		return industryService.isAlreadExistIndustry(industry);
	}

	public void addNewIndustry() {
		try {
			if (!isAlreadyExistIndustry()) {
				industryService.addNewIndustry(industry);
				addInfoMessage(null, MessageId.INSERT_SUCCESS, industry.getName());
				createNewIndustry();
				loadIndustry();
			} else {
				addInfoMessage(null, MessageId.ALREADY_ADD_INDUSTRY, industry.getName());
			}
