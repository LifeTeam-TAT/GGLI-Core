...
public class ManageOccupationActionBean extends BaseBean implements Serializable {
	...
	public void createNewOccupation() {
		...
		occupation = new Occupation();
		criteria = "";
		PrimeFaces.current().resetInputs(":occupationEntryForm");
	}
	...
	public boolean isAlreadyExistOccupation() {
		return occupationService.isAlreadyExistOccupation(occupation);
	}

	public void addNewOccupation() {
		try {
			if (!isAlreadyExistOccupation()) {
				occupationService.addNewOccupation(occupation);
				addInfoMessage(null, MessageId.INSERT_SUCCESS, occupation.getName());
				createNewOccupation();
				loadOccupation();
			} else {
				addInfoMessage(null, MessageId.ALREADY_ADD_OCCCUPATION, occupation.getName());
			}
